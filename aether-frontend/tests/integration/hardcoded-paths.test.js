...
describe('Hardcoded Paths Validation', () => {
  ...
  const ALLOWED_PATTERNS = [
    ...
    // (LM Studio, Ollama, Perplexica) presented in UI for override
    'modules/settings/LLMProviderSettings.js',
    'modules/settings/UserCredentialsSettings.js',
    // Agent modals contain Perplexica dashboard URL defaults
    'modules/agents/AgentsModal.js',
    'research/index.js',
  ...
  beforeAll(() => {
    // Find all JS files in src directory (sync to guarantee array)
    const pattern = path.join(SRC_DIR, '**/*.js').replace(/\\/g, '/');
    sourceFiles = globSync(pattern, { nodir: true });
  });
  ...
  test('should not have excessive relative imports', () => {
    ...
    // Ratchet: deep imports (../../../../) are a code smell but widespread.
    // Track the ceiling so new deep imports don't silently accumulate.
    // Baseline: 101 as of 2026-02-24.
    const KNOWN_CEILING = 101;
