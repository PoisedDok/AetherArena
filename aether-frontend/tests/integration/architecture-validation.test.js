...
describe('Architecture Validation', () => {
  ...
  describe('Clean architecture layers', () => {
    ...
    it('should use dependency injection instead of direct imports', () => {
      const orchestrators = [
        ...
        'application/main/ArtifactsStreamOrchestrator.js'
      ];
  ...
  describe('Separation of concerns', () => {
    ...
    it('should use centralized configuration', () => {
      ...
      // Files allowed to have inline URLs:
      // - core/config: centralized config defaults
      // - Settings modules: contain user-configurable third-party service defaults
      //   (Ollama, LM Studio, Perplexica) that are presented in the UI for override
      // - Agent modals: contain Perplexica dashboard URL defaults
      const allowedInlineUrlPaths = [
        'core/config',
        'modules/settings/LLMProviderSettings',
        'modules/settings/UserCredentialsSettings',
        'modules/agents/AgentsModal',
        'research/index.js',
      ...
      files.forEach(file => {
        ...
        if (inlineUrls && !allowedInlineUrlPaths.some(p => relativePath.includes(p))) {
          violations.push({
            file: relativePath,
            urls: inlineUrls
          });
  ...
  describe('Code quality', () => {
    it('should use strict mode', () => {
      ...
      // Should have most files in strict mode (>=75% compliance)
      expect(violations.length / files.length).toBeLessThanOrEqual(0.25);
    });
