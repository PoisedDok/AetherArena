...
describe('Config Resolvers', () => {
  ...
  describe('resolveUrl', () => {
    test('should return default when no overrides', () => {
      const url = resolveUrl('TEST_URL', 'test_url', 'http://localhost:8765');
    ...
    test('should normalize URLs by removing trailing slash', () => {
      process.env.TEST_URL = 'http://localhost:8765/';
  ...
  describe('resolveFullUrl', () => {
    ...
    test('should throw when baseUrl is missing', () => {
      expect(() => resolveFullUrl(null, '/api/test')).toThrow('Both baseUrl and endpoint are required');
    });

    test('should throw when endpoint is missing', () => {
      expect(() => resolveFullUrl('http://localhost:8765', null)).toThrow('Both baseUrl and endpoint are required');
    });
  });

  describe('resolveWsUrl — error path', () => {
    test('should throw when httpUrl is empty', () => {
      expect(() => resolveWsUrl('')).toThrow('HTTP URL required');
    });

    test('should throw when httpUrl is null', () => {
      expect(() => resolveWsUrl(null)).toThrow('HTTP URL required');
    });
  });

  describe('resolveUrl — edge cases', () => {
    test('returns empty default as-is when no overrides', () => {
      const result = resolveUrl('NONEXISTENT_KEY', 'nonexistent_ls', '');
      expect(result).toBe('');
    });

    test('returns null default as-is when no overrides', () => {
      const result = resolveUrl('NONEXISTENT_KEY', 'nonexistent_ls', null);
  ...
  describe('resolveString', () => {
    test('returns default when no overrides', () => {
      const result = resolveString('NONEXISTENT_KEY', 'nonexistent_ls', 'hello');
      expect(result).toBe('hello');
    });

    test('uses localStorage override when set', () => {
      global.window.localStorage.setItem('test_str', 'stored');
      const result = resolveString('NONEXISTENT_KEY', 'test_str', 'default');
      expect(result).toBe('stored');
    });

    test('uses env variable when localStorage not set', () => {
      process.env.TEST_STR = 'from-env';
      const result = resolveString('TEST_STR', 'nonexistent_ls', 'default');
  ...
  describe('resolvePort', () => {
    test('returns default when no overrides', () => {
      const result = resolvePort('NONEXISTENT_KEY', 'nonexistent_ls', 8000);
      expect(result).toBe(8000);
    });

    test('parses valid port from env', () => {
      process.env.TEST_PORT = '3000';
      const result = resolvePort('TEST_PORT', 'test_port', 8000);
      expect(result).toBe(3000);
    });

    test('enforces port range (1-65535)', () => {
  ...
  describe('resolveFileSize', () => {
    test('returns default when no overrides', () => {
      const result = resolveFileSize('NONEXISTENT_KEY', 'nonexistent_ls', 1048576);
      expect(result).toBe(1048576);
    });

    test('parses valid file size from env', () => {
      process.env.TEST_SIZE = '2097152';
      const result = resolveFileSize('TEST_SIZE', 'test_size', 1048576);
      expect(result).toBe(2097152);
    });
  });

  describe('resolveInt — localStorage path', () => {
    test('uses localStorage value when valid', () => {
      global.window.localStorage.setItem('test_int_ls', '25');
      const result = resolveInt('NONEXISTENT_KEY', 'test_int_ls', 10, 1, 100);
      expect(result).toBe(25);
    });
  });

  describe('resolveLogLevel — localStorage path', () => {
    test('uses localStorage override', () => {
      global.window.localStorage.setItem('test_log_ls', 'debug');
      const result = resolveLogLevel('NONEXISTENT_KEY', 'test_log_ls', 'info');
      expect(result).toBe('debug');
    });

    test('returns validated default when no overrides', () => {
      const result = resolveLogLevel('NONEXISTENT_KEY', 'nonexistent_ls', 'warn');
      expect(result).toBe('warn');
    });
  });

  describe('resolveSanitizerProfile — localStorage path', () => {
    test('uses localStorage override', () => {
      global.window.localStorage.setItem('test_san_ls', 'permissive');
      const result = resolveSanitizerProfile('NONEXISTENT_KEY', 'test_san_ls', 'strict');
      expect(result).toBe('permissive');
    });

    test('returns validated default when no overrides', () => {
      const result = resolveSanitizerProfile('NONEXISTENT_KEY', 'nonexistent_ls', 'default');
      expect(result).toBe('default');
    });
  });

  describe('resolveStorageBackend — localStorage path', () => {
    test('uses localStorage override', () => {
      global.window.localStorage.setItem('test_be_ls', 'sqlite');
      const result = resolveStorageBackend('NONEXISTENT_KEY', 'test_be_ls', 'postgresql');
      expect(result).toBe('sqlite');
    });

    test('returns validated default when no overrides', () => {
      const result = resolveStorageBackend('NONEXISTENT_KEY', 'nonexistent_ls', 'memory');
      expect(result).toBe('memory');
    });
  });

  describe('getLocalStorageValue — edge cases', () => {
    test('returns null when window is undefined', () => {
      const origWindow = global.window;
      delete global.window;

      // resolveBoolean uses getLocalStorageValue internally
      const result = resolveBoolean('NONEXISTENT_KEY', 'nonexistent_ls', true);
      expect(result).toBe(true);

      global.window = origWindow;
    });

    test('returns null and warns when localStorage throws', () => {
      const origGetItem = global.window.localStorage.getItem;
      global.window.localStorage.getItem = () => { throw new Error('Quota exceeded'); };
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation();

      const result = resolveBoolean('NONEXISTENT_KEY', 'test_bool_err', false);
      expect(result).toBe(false);
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to read localStorage'), expect.any(String));

      warnSpy.mockRestore();
      global.window.localStorage.getItem = origGetItem;
    });
  });

  describe('resolveBoolean — default fallthrough', () => {
    test('returns default when no env and no localStorage', () => {
      const result = resolveBoolean('NONEXISTENT_KEY', 'nonexistent_ls', true);
      expect(result).toBe(true);
    });
  });
