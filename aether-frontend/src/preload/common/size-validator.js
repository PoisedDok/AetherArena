...
const DEFAULT_SIZE_LIMITS = freeze({
  ...
  // Per-channel limits
  channels: freeze({
    // Large content channels (increased for agentic system with large tool catalogs)
    'artifacts:stream': 10 * 1024 * 1024, // 10MB (agent-generated artifacts)
    'artifacts:load-code': 10 * 1024 * 1024, // 10MB (code artifacts)
    'artifacts:load-output': 10 * 1024 * 1024, // 10MB (output artifacts)
    'artifacts:file-export': 20 * 1024 * 1024, // 20MB (file exports)
    'chat:send': 500 * 1024, // 500KB (user input + context)
    'artifacts:execute-code': 500 * 1024, // 500KB (code execution)
    
    // Agent streaming channels (CRITICAL: increased for AGI system prompts + tool catalogs)
    'chat:assistant-stream': 2 * 1024 * 1024, // 2MB per chunk (GURU system prompt + 220 tools catalog)
    'widget-drag-start': 1024, // 1KB
    'widget-drag-move': 1024, // 1KB
    'widget-drag-end': 1024, // 1KB
    'renderer-log': 100 * 1024, // 100KB (detailed logs)
    
    // Control channels
    'chat:window-control': 10 * 1024, // 10KB
    'artifacts:window-control': 10 * 1024, // 10KB
    'chat:stop': 10 * 1024, // 10KB
    'zoom-in': 1024, // 1KB
    'zoom-out': 1024, // 1KB
