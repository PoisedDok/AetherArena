...
function resolveUrl(envKey, localStorageKey, defaultValue) {
  ...
  // 3. Use default.
  // Fail-fast support: defaults may intentionally be empty to force env/central config.
...
/**
 * Resolve string with precedence: localStorage > env > default
 * @param {string} envKey - Environment variable key
 * @param {string} localStorageKey - localStorage key
 ...
 */
function resolveString(envKey, localStorageKey, defaultValue, maxLength = Infinity) {
  const override = getLocalStorageValue(localStorageKey);
  if (override !== null) {
    return validateString(override, defaultValue, maxLength);
  }

  const envValue = envLoader.getString(envKey);
  if (envValue !== undefined && envValue !== null && envValue !== '') {
    return validateString(envValue, defaultValue, maxLength);
  }

  return validateString(defaultValue, defaultValue, maxLength);
}
