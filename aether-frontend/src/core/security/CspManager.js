...
const DEFAULT_POLICIES = freeze({
  ...
  production: freeze({
    ...
    'media-src': ["'self'", 'blob:'],
    // SECURITY: never allow plugins/embeds in production
    'object-src': ["'none'"],
    'base-uri': ["'self'"],
    'form-action': ["'self'"],
  ...
  development: freeze({
    ...
    'font-src': ["'self'", 'data:'],
    'connect-src': ["'self'", 'ws:', 'wss:', 'http:', 'https:', 'http://localhost:*', 'http://127.0.0.1:*'],
    'media-src': ["'self'", 'blob:'],
    // Dev-only: keep permissive for debugging (Electron apps rarely need plugins anyway).
    'object-src': ['data:'],
    'base-uri': ["'self'"],
    'form-action': ["'self'"],
...
class CspManager {
  constructor(options = {}) {
    ...
    this.log = createLogger({ component: 'CspManager' });
  ...
  generateNonce() {
    if (typeof window !== 'undefined' && window.crypto) {
      ...
      // Node.js main process: use crypto module (always available)
      try {
        const nodeCrypto = require('crypto');
        this.currentNonce = nodeCrypto.randomBytes(16).toString('hex');
      } catch (_) {
        // SECURITY: Never fall back to Math.random() for nonces.
        // If crypto is unavailable, fail loudly rather than silently produce weak nonces.
        throw new Error('[CspManager] No cryptographic random source available for nonce generation');
      }
    }
   ...
   * @param {Array<string>} [options.omitDirectives] - Directives to omit from output (e.g. frame-ancestors for meta tags)
   * @returns {string}
   */
  buildPolicy(options = {}) {
    const omit = new Set(Array.isArray(options.omitDirectives) ? options.omitDirectives : []);
    const parts = [];
    
    for (const [directive, values] of Object.entries(this.directives)) {
      if (omit.has(directive)) continue;
      if (values.length > 0) {
  ...
  /**
   * Build CSP suitable for injection via <meta http-equiv="Content-Security-Policy">.
   * NOTE: Some directives (e.g. frame-ancestors) are ignored when delivered via meta in Chromium.
   * @returns {string}
   */
  buildMetaPolicy() {
    return this.buildPolicy({ omitDirectives: ['frame-ancestors'] });
  }
  ...
  injectMetaTag(doc = typeof window !== 'undefined' ? window.document : null) {
    ...
    meta.setAttribute('content', this.buildMetaPolicy());
  ...
  _recordViolation(violation) {
    ...
    if (this.environment === 'development') {
      this.log.warn('CSP violation detected', { violation });
    }
  ...
  async _reportViolation(violation) {
    try {
      ...
      this.log.error('Failed to report violation', { error: error?.message });
    }
  }
