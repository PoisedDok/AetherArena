...
const SQL_STRICT_PATTERNS = [
  /(['"])\s*(?:or|and)\s*['"]?\s*(?:1\s*=\s*1|true)\b/i,
  /(['"])\s*(?:or|and)\s*['"]?\s*\w+\s*=\s*\w+/i,
  /\bunion\s+all?\s+select\b/i,
  /;\s*(?:drop|truncate|delete)\s+(?:table|database)\b/i,
  /\b(?:drop|create|alter)\s+database\b/i,
  /\binto\s+outfile\b/i,
  /\b(?:exec|execute)\s+(?:xp_|sp_)?[a-z_]+\b/i,
  /\b(?:sleep|benchmark|load_file)\s*\(/i
];

const SQL_BOOLEAN_COMPARISON_PATTERN = /\b(?:or|and)\b[^;]*?(?:=|<>|!=|<=|>=)/i;

function detectHighRiskSql(rawValue) {
  ...
  for (const pattern of SQL_STRICT_PATTERNS) {
  ...
  // Note: comment markers (--,/*) and terminators (;) are already caught by
  // the broad VALIDATION_RULES.sqlInjection regex BEFORE detectHighRiskSql
  // is called.  Only keyword-count and boolean-comparison checks are
  // reachable here.
  const hasBooleanComparison = SQL_BOOLEAN_COMPARISON_PATTERN.test(value);

  if (keywordHits >= 2 && hasBooleanComparison) {
    return true;
  }
...
class InputValidator {
  ...
  validateString(value, constraints = {}) {
    ...
    if (constraints.noSqlInjection) {
      // Broad pattern first (catches common cases like 'SELECT * FROM users')
      VALIDATION_RULES.sqlInjection.lastIndex = 0;
      if (VALIDATION_RULES.sqlInjection.test(value) || detectHighRiskSql(value)) {
        this.stats.failures++;
        throw new ValidationError(
  ...
  validate(data, schema) {
    ...
    for (const [field, rules] of Object.entries(schema)) {
      ...
      // Note: email format validation is handled entirely by _validateFieldType
      // which uses the same VALIDATION_RULES.email regex.  No redundant check needed.

      // URL format — catches URLs that pass isValidUrl() but fail validateURL()
      // (e.g. ftp:// passes dangerous-protocol check but is not in allowedProtocols)
...
// SECURITY: Do NOT expose security classes globally.
// Consumers must use require() / DI container to access InputValidator.
