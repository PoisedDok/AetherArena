...
const processTypeBootstrap = (() => {
  if (hasProcess && process?.type) {
    return process.type;
  }
...
const isRendererProcess = processTypeBootstrap === 'renderer';
let fs = null;
let path = null;
...
function parseLogLevel(value) {
  ...
  if (typeof value === 'string' && value) {
    return LOG_LEVELS[value.toUpperCase()] ?? DEFAULT_CONFIG.level;
  }
  return DEFAULT_CONFIG.level;
}

function resolveRendererLogLevel(appConfig, configuredLevel) {
  if (!isRendererProcess) {
    return configuredLevel;
  }
  const verboseEnabled = Boolean(appConfig?.dev?.verboseLogging || appConfig?.dev?.debugMode);
  if (verboseEnabled) {
    return configuredLevel;
  }
  return configuredLevel > LOG_LEVELS.WARN ? LOG_LEVELS.WARN : configuredLevel;
}

function buildLogConfig(appConfig, overrides = {}) {
  const hasLevelOverride = typeof overrides.level === 'number';
  const configuredLevel = parseLogLevel(appConfig?.logging?.level);
  const effectiveLevel = hasLevelOverride
    ? overrides.level
    : resolveRendererLogLevel(appConfig, configuredLevel);

  return {
    ...DEFAULT_CONFIG,
    ...overrides,
    level: effectiveLevel,
    console: typeof overrides.console === 'boolean'
      ? overrides.console
      : (appConfig?.logging?.console ?? DEFAULT_CONFIG.console),
    file: typeof overrides.file === 'boolean'
      ? overrides.file
      : (appConfig?.logging?.file ?? DEFAULT_CONFIG.file),
    maxFileSize: overrides.maxFileSize ?? appConfig?.logging?.maxFileSize ?? DEFAULT_CONFIG.maxFileSize,
    maxFiles: overrides.maxFiles ?? appConfig?.logging?.maxFiles ?? DEFAULT_CONFIG.maxFiles,
  };
}
...
class Logger {
  constructor(config = {}) {
    ...
    this.pid = hasProcess && process?.pid !== undefined ? process.pid : Date.now();
    this.supportsFileLogging = !isRendererProcess && !!fs && !!path;
  ...
  _initializeLogDirectory() {
    ...
    try {
      ...
      if (!logDir) {
        ...
        } else if (hasProcess && typeof process.cwd === 'function') {
...
function getLogger(config = {}) {
  if (!globalLogger) {
    ...
    try {
      ...
      const logConfig = buildLogConfig(appConfig, config);
      globalLogger = new Logger(logConfig);
    } catch {
      // Fallback to default config if unified config not available
      globalLogger = new Logger({ ...DEFAULT_CONFIG, ...config });
    }
  }
  return globalLogger;
}
