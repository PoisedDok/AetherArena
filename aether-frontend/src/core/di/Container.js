...
class DependencyContainer {
  constructor(options = {}) {
    ...
    this.log = createLogger({ component: `DI:${this.name}` });
  }
  ...
  register(name, factory, options = {}) {
    ...
    if (this.enableLogging) {
      this.log.debug('registered', { name, type: options.singleton !== false ? 'singleton' : 'transient' });
    }
  }
  ...
  resolve(name) {
    ...
    try {
      ...
      if (this.enableLogging) {
        this.log.debug('resolved', { name });
      }

      return instance;

    } catch (error) {
      this.log.error('failed to resolve', { name, error: error.message });
      throw error;
    } finally {
  ...
  dispose() {
    ...
    if (this.enableLogging) {
      this.log.debug('disposing', { singletonCount: this._singletons.size });
    }
    ...
    for (const [name, instance] of singletons) {
      if (instance && typeof instance.dispose === 'function') {
        try {
          instance.dispose();
          if (this.enableLogging) {
            this.log.debug('disposed service', { name });
          }
        } catch (error) {
          this.log.error('failed to dispose', { name, error: error.message });
        }
      }
    }
    ...
    if (this.enableLogging) {
      this.log.debug('container disposed');
    }
  }
  ...
  reset() {
    ...
    if (this.enableLogging) {
      this.log.debug('container reset');
    }
  }
...
// SECURITY: Do NOT expose DI container class globally.
// Consumers must use require() to access DependencyContainer.
