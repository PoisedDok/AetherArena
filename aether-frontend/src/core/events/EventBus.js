...
class EventBus {
  constructor(options = {}) {
    ...
    this.log = createLogger({ component: `EventBus:${options.name || 'default'}` });
  ...
  on(eventName, handler, options = {}) {
    if (this._destroyed) {
      this.log.warn('cannot subscribe after destruction');
      return () => {};
    }
    ...
    if (this.enableLogging) {
      this.log.debug('subscribed', { event: eventName, id: subscription.id, priority: subscription.priority });
    }
  ...
  off(eventName, handlerOrId) {
    ...
    if (index !== -1) {
      ...
      if (this.enableLogging) {
        this.log.debug('unsubscribed', { event: eventName });
      }
    }
  ...
  emit(eventName, data, meta = {}) {
    if (this._destroyed) {
      this.log.warn('cannot emit after destruction');
      return;
    }
    ...
    if (this.validators.has(eventName)) {
      ...
      if (!result.valid) {
        this.log.error('event validation failed', { event: eventName, errors: result.errors });
        return;
      }
    }
    ...
    for (const mw of this.middleware) {
      try {
        ...
        this.log.error('middleware error', { error: error.message });
      }
    }
    ...
    if (this.enableLogging) {
      this.log.debug('emitting', { event: eventName });
    }
    ...
    if (!subscribers || subscribers.length === 0) {
      if (this.enableLogging) {
        this.log.debug('no subscribers', { event: eventName });
      }
      return;
    }
    ...
    for (const sub of subscribersCopy) {
      ...
      try {
        ...
        this.log.error('handler error', { event: eventName, error: error.message });
      }
    }
...
// SECURITY: Do NOT expose EventBus class globally.
// Consumers must use require() / DI container to access EventBus.
