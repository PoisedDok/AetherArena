 ...
 * Processing: Date formatting, relative time calculation, duration formatting, timestamp conversions --- {2 jobs: JOB_CALCULATE_TIME, JOB_FILTER_DATA}
...
const _dateLog = (typeof require !== 'undefined')
  ? require('./logger').createRendererLogger('DateUtils')
...
const DateUtils = freeze({
  ...
  format(date, options = {}) {
    try {
      ...
      _dateLog.error('[DateUtils] format failed:', error);
      return '';
    }
  },
  ...
  formatTime(date, options = {}) {
    try {
      ...
      _dateLog.error('[DateUtils] formatTime failed:', error);
      return '';
    }
  },
  ...
  formatDateTime(date, options = {}) {
    try {
      ...
      _dateLog.error('[DateUtils] formatDateTime failed:', error);
      return '';
    }
  },
  ...
  toISO(date) {
    try {
      ...
      _dateLog.error('[DateUtils] toISO failed:', error);
      return '';
    }
  },
  ...
  relative(date) {
    try {
      ...
      _dateLog.error('[DateUtils] relative failed:', error);
      return '';
    }
  },
  ...
  formatDuration(ms, verbose = false) {
    try {
      ...
      _dateLog.error('[DateUtils] formatDuration failed:', error);
      return '0ms';
    }
  },
   ...
   * @param {number|null} endTime - End timestamp (ms). Defaults to now.
   * @param {Object} options - Options { verbose?: boolean }
   * @returns {string} Formatted elapsed string
   */
  formatElapsedTime(startTime, endTime = null, options = {}) {
    try {
      ...
      return this.formatDuration(duration, options.verbose);
    } catch (error) {
      _dateLog.error('[DateUtils] formatElapsedTime failed:', error);
      return '0ms';
    }
  },
  ...
  getTimestamp(date = null) {
    try {
      ...
      _dateLog.error('[DateUtils] getTimestamp failed:', error);
      return Date.now();
    }
  },
  ...
  add(date, amount, unit = 'ms') {
    try {
      ...
      _dateLog.error('[DateUtils] add failed:', error);
      return new Date();
    }
  },
...
if (typeof window !== 'undefined') {
  ...
  _dateLog.debug('📦 DateUtils loaded');
}
