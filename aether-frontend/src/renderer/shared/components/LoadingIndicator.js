 ...
 * Processing: Create DOM with CSS animations (4 styles: spinner/pulse/dots/skeleton), inject styles, show/hide with fade, update progress bar (0-100), update message text, accessibility (aria-live polite) --- {2 jobs: JOB_CREATE_DOM_ELEMENT, JOB_UPDATE_DOM_ELEMENT}
...
class LoadingIndicator {
   ...
   */
  constructor(options = {}) {
    this.log = createRendererLogger('LoadingIndicator');
    // Configuration
    ...
    this.color = options.color || 'var(--color-accent)';
    ...
    this.log.debug('[LoadingIndicator] Constructed');
  }
   ...
   */
  init() {
    this.log.debug('[LoadingIndicator] Initializing...');
    ...
    this.log.debug('[LoadingIndicator] Initialized');
  }
  ...
  show(message = null, progress = null) {
    ...
    // Show element
    if (this.overlay && this.element.tagName === 'DIALOG') {
    ...
    this.log.debug('[LoadingIndicator] Shown');
  }
  ...
  hide() {
    ...
    setTimeout(() => {
      if (this.element && !this.element.classList.contains('visible')) {
        ...
        // Always remove overlays to avoid stacking and top layer pollution
        if (this.overlay && this.element.parentNode) {
    ...
    this.log.debug('[LoadingIndicator] Hidden');
  }
  ...
  _createElement() {
    ...
    if (this.overlay) {
      ...
      this.element.className = `${CONFIG.CLASS_NAMES.CONTAINER} ${CONFIG.CLASS_NAMES.OVERLAY}`;
    } else {
  ...
  _injectStyles() {
    ...
    style.textContent = `
      ...
      .${CONFIG.CLASS_NAMES.SKELETON} {
        width: 200px;
        height: 20px;
        background: linear-gradient(90deg, var(--color-surface-base) 25%, var(--color-surface-hover) 50%, var(--color-surface-base) 75%);
  ...
  dispose() {
    ...
    this.log.debug('[LoadingIndicator] Disposing...');
    ...
    this.log.debug('[LoadingIndicator] Disposed');
  }
}
