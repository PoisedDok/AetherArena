...
const _log = createRendererLogger('RequestLifecycleManager');
...
class RequestLifecycleManager {
  ...
  startRequest(options = {}) {
    ...
    if (this.enableLogging) {
      _log.debug(`[${this.name}] Started request ${requestId} (type: ${context.type})`);
    }
  ...
  completeRequest(requestId, result = null) {
    ...
    if (!context) {
      _log.warn(`[${this.name}] Cannot complete unknown request: ${requestId}`);
      return;
    }
    ...
    if (context.onComplete) {
      try {
        ...
        _log.error(`[${this.name}] onComplete callback failed for ${requestId}:`, error);
      }
    }
    ...
    if (this.enableLogging) {
      _log.debug(`[${this.name}] Completed request ${requestId} (${duration}ms)`);
    }
  }
  ...
  failRequest(requestId, error) {
    const context = this.activeRequests.get(requestId);
    if (!context) {
      _log.warn(`[${this.name}] Cannot fail unknown request: ${requestId}`);
      return;
    }
    ...
    _log.error(`[${this.name}] Failed request ${requestId} (${duration}ms):`, error);
  }
  ...
  cancelRequest(requestId) {
    const context = this.activeRequests.get(requestId);
    if (!context) {
      _log.warn(`[${this.name}] Cannot cancel unknown request: ${requestId}`);
      return false;
    }
    ...
    if (context.onCancel) {
      try {
        ...
        _log.error(`[${this.name}] onCancel callback failed for ${requestId}:`, error);
      }
    }
    ...
    if (this.enableLogging) {
      _log.debug(`[${this.name}] Cancelled request ${requestId} (${duration}ms)`);
    }
  ...
  cancelAll() {
    ...
    if (this.enableLogging) {
      _log.debug(`[${this.name}] Cancelled all requests (${count})`);
    }
  ...
  reset() {
    ...
    if (this.enableLogging) {
      _log.debug(`[${this.name}] Reset statistics`);
    }
  }
  ...
  destroy() {
    ...
    if (this.enableLogging) {
      _log.debug(`[${this.name}] Destroyed`);
    }
  }
  ...
  _handleTimeout(requestId) {
    ...
    if (context.onTimeout) {
      try {
        ...
        _log.error(`[${this.name}] onTimeout callback failed for ${requestId}:`, error);
      }
    }
    ...
    _log.warn(`[${this.name}] Request ${requestId} timed out after ${duration}ms`);
  }
...
if (typeof window !== 'undefined') {
  ...
  _log.debug('RequestLifecycleManager loaded');
}
