 ...
 * 
 * @module application/main/modules/profiles/ProfileManager
 * 
...
const _log = createRendererLogger('ProfileManager');
...
class ProfileManager {
  ...
  async refreshProfileList() {
    try {
      if (this.enableLogging) {
        _log.debug('[ProfileManager] Refreshing profile list...');
      }
      ...
      if (this.enableLogging) {
        _log.debug(`[ProfileManager] Found ${this.profiles.length} profiles`);
      }

      return this.profiles;
    } catch (error) {
      _log.error('[ProfileManager] Error refreshing profile list:', error);
      return [];
    }
  }
  ...
  async setCurrentProfile(profileName) {
    try {
      ...
      if (this.enableLogging) {
        _log.debug(`[ProfileManager] Setting profile: ${profileName}`);
      }
      ...
      if (this.enableLogging) {
        _log.debug(`[ProfileManager] Profile changed: ${previousProfile} → ${profileName}`);
      }

      return true;
    } catch (error) {
      _log.error('[ProfileManager] Error setting profile:', error);
      return false;
    }
  }
  ...
  getDefaultProfile() {
    ...
    return this.profiles[0] || 'GURU.py';
  }
  ...
  dispose() {
    ...
    if (this.enableLogging) {
      _log.debug('[ProfileManager] Disposed');
    }
  }
}
...
if (typeof window !== 'undefined') {
  ...
  _log.debug('ProfileManager loaded');
}
