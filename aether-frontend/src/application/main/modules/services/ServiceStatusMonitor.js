 ...
 * 
 * @module application/main/modules/services/ServiceStatusMonitor
 * 
...
const _log = createRendererLogger('ServiceStatusMonitor');
...
class ServiceStatusMonitor {
  constructor(options = {}) {
    ...
    // Configuration
    this.checkInterval = options.checkInterval || 30000; // Increased from 4s
    this.timeout = options.timeout || 10000; // Increased from 2.5s
  ...
  registerService(key, config) {
    ...
    if (this.enableLogging) {
      _log.debug(`[ServiceStatusMonitor] Registered service: ${key}`);
    }
  }
  ...
  start() {
    if (this.intervalId) {
      _log.warn('[ServiceStatusMonitor] Already started');
      return;
    }
    ...
    if (this.enableLogging) {
      _log.debug(`[ServiceStatusMonitor] Started (interval: ${this.checkInterval}ms)`);
    }
  }
  ...
  stop() {
    if (this.intervalId) {
      ...
      if (this.enableLogging) {
        _log.debug('[ServiceStatusMonitor] Stopped');
      }
    }
  }
  ...
  async checkAll() {
    try {
      ...
      if (this.enableLogging) {
        _log.error('[ServiceStatusMonitor] Failed to run health checks:', error);
      }
    }
  }
  ...
  async checkService(key) {
    ...
    try {
      // ALL services now checked through backend gateway (port 8765)
      // No direct service access allowed
      if (!this.endpoint) {
        throw new Error('Backend endpoint not available');
      }
      
      if (service.useEndpoint || key === 'backend') {
        // Backend health check
        await this.endpoint.getHealth();
        newStatus = 'ok';
      } else {
        // Service health check through backend proxy
        // Backend should expose: GET /v1/api/services/{service}/health
        try {
          const response = await this.endpoint.api.get(
            `/v1/api/services/${key}/health`,
            { timeout: this.timeout }
          );
          newStatus = response && response.status === 'ok' ? 'ok' : 'warn';
        } catch (error) {
          // Service may not have health endpoint yet - log warning
          if (this.enableLogging) {
            _log.warn(`[ServiceStatusMonitor] Service health check not available for ${key} - backend should implement /v1/api/services/${key}/health`);
          }
          throw error;
        }
      }
      ...
      if (this.enableLogging) {
        _log.warn(`[ServiceStatusMonitor] Check failed for ${key}:`, error.message);
      }
    }
  ...
  _updateServiceStatus(key, newStatus) {
    ...
    if (this.enableLogging && oldStatus !== newStatus) {
      _log.debug(`[ServiceStatusMonitor] ${key}: ${oldStatus} → ${newStatus}`);
    }
  }
  ...
  dispose() {
    ...
    if (this.enableLogging) {
      _log.debug('[ServiceStatusMonitor] Disposed');
    }
  }
}
...
if (typeof window !== 'undefined') {
  ...
  _log.debug('ServiceStatusMonitor loaded');
}
