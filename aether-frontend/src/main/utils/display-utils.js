...
const log = createLogger({ component: 'DisplayUtils' });
...
function calculateWidgetPosition(currentBounds, widgetSize, margin = 24) {
  try {
    ...
    log.error('failed to calculate widget position', { error: err.message });
...
function calculateCenteredPosition(width, height, display = null) {
  try {
    ...
    log.error('failed to calculate centered position', { error: err.message });
...
function ensureBoundsVisible(bounds) {
  try {
    ...
    log.error('failed to ensure bounds visible', { error: err.message });
    return bounds;
  }
}
...
function constrainToWorkArea(bounds, display = null) {
  try {
    ...
    log.error('failed to constrain to work area', { error: err.message });
    return bounds;
  }
}
...
function calculateOffsetPosition(anchorBounds, width, height, offset = { x: 20, y: 20 }) {
  try {
    ...
    log.error('failed to calculate offset position', { error: err.message });
    return { x: 100, y: 100, width, height };
  }
}
...
function getCursorPosition() {
  try {
    ...
    log.error('failed to get cursor position', { error: err.message });
    return { x: 0, y: 0 };
  }
}
...
function getDisplayAtCursor() {
  try {
    ...
    log.error('failed to get display at cursor', { error: err.message });
...
function calculateOptimalPosition(width, height, offset = { x: 10, y: 10 }) {
  try {
    ...
    log.error('failed to calculate optimal position', { error: err.message });
...
function getScaleFactor(display = null) {
  try {
    ...
    log.error('failed to get scale factor', { error: err.message });
    return 1.0;
  }
}
...
function isWithinDisplay(bounds, display = null) {
  try {
    ...
    log.error('failed to check if within display', { error: err.message });
    return false;
  }
}
...
function getWorkArea(display = null) {
  try {
    ...
    log.error('failed to get work area', { error: err.message });
