...
// CRITICAL: Do NOT import logger here - circular dependency
// logger -> config -> PortResolver -> PortManager -> logger
...
const PORT_RANGES = Object.freeze({
  // Frontend should ONLY discover the backend (bootstrap), not internal/in-process capabilities.
  // All other service URLs/status come from backend central config via GET /v1/services/status.
  backend: { start: 8765, end: 8775 },
  
  // Perplexica research service (for iframe embedding in agents modal)
  // This range is whitelisted in SecurityManager to allow HTTP iframe loading
...
const HEALTH_ENDPOINTS = Object.freeze({
  // Backend health endpoint (FastAPI): prefer versioned health route.
  backend: DEFAULTS.endpoints.health,
});
...
class PortManager {
  constructor(options = {}) {
    this.options = {
      // Spread raw options FIRST as a base, then override with computed/merged values.
      // Previously `...options` was at the END, which overwrote the merged portRanges
      // and healthEndpoints with the raw (un-merged) values — a correctness bug.
      ...options,
      ...
      // Host/protocol are part of PortManager's responsibility (service discovery).
      // Do NOT depend on core/config here; core/config depends on PortResolver which depends on PortManager.
      protocol: options.protocol || 'http',
      host: options.host || '127.0.0.1',
    };
    
    // Use minimal logging (avoid circular dependency with logger)
    this.logger = {
      debug: (...args) => process.env.DEBUG && console.log('[PortManager:DEBUG]', ...args),
      info: (...args) => console.log('[PortManager:INFO]', ...args),
      warn: (...args) => console.warn('[PortManager:WARN]', ...args),
      error: (...args) => console.error('[PortManager:ERROR]', ...args),
    };
  ...
  _buildServiceUrl(port) {
    return `${this.options.protocol}://${this.options.host}:${port}`;
  }
  ...
  async discoverService(serviceName) {
    const range = this.options.portRanges[serviceName];
    const healthEndpoint = this.options.healthEndpoints[serviceName] || DEFAULTS.endpoints.health;
    ...
    for (let i = 0; i < ports.length; i += this.options.maxConcurrentChecks) {
      ...
      const results = await Promise.all(
        batch.map(async (port) => {
          const url = this._buildServiceUrl(port);
  ...
  /**
   * Get all service statuses for UI display
   * Includes both discovered and configured (but not running) services
   * @returns {Array} Array of service status objects
   */
  getServiceStatus() {
    const statuses = [];
    const allServiceNames = Object.keys(this.options.portRanges);
    ...
    for (const serviceName of allServiceNames) {
      ...
      if (service) {
        // Service discovered and registered
        statuses.push({
          name: serviceName,
          url: service.url,
          port: service.port,
        ...
        const range = this.options.portRanges[serviceName];
        const expectedPort = range.start;
        const expectedUrl = this._buildServiceUrl(expectedPort);
