 ...
 * - Alt+T: Toggle cosmos demo (visual showcase + welcome text)
 * 
...
class ShortcutManager {
  ...
  async initialize() {
    ...
    try {
      ...
      this._registerToggleNotes();
      this._registerToggleDemo();
  ...
  _registerToggleArtifacts() {
    this._registerShortcuts(
      ...
      () => {
        this.windowManager.controlArtifactsWindow('toggle-visibility');
      },
  ...
  /**
   * Register toggle notes window shortcuts
   */
  _registerToggleNotes() {
    this._registerShortcuts(
      this.options.shortcuts.TOGGLE_NOTES,
      () => {
        this.windowManager.controlNotesWindow('toggle-visibility');
      },
      'Toggle notes window'
    );
  }
  ...
  /**
   * Register toggle cosmos demo shortcut.
   * Sends 'demo:toggle' IPC to the main window renderer which owns the
   * visualizer and welcome demo lifecycle.
   */
  _registerToggleDemo() {
    this._registerShortcuts(
      this.options.shortcuts.TOGGLE_DEMO,
      () => {
        const mainWin = this.windowManager.getMainWindow();
        if (!mainWin || mainWin.isDestroyed()) return;

        try {
          mainWin.webContents.send('demo:toggle');
        } catch (err) {
          this.logger.error('Failed to send demo:toggle to main window', {
            error: err.message,
          });
        }
      },
