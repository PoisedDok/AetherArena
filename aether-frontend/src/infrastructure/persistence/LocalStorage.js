...
const _log = createRendererLogger('LocalStorage');
...
class LocalStorage {
  constructor(options = {}) {
    ...
    if (!this.available) {
      _log.warn('[LocalStorage] localStorage not available, using in-memory fallback');
  ...
  set(key, value) {
    ...
    // BUG 10 FIX: serialized must be hoisted out of try so the QuotaExceededError
    // retry in catch can access it. Previously const inside try was block-scoped
    // and the retry always threw ReferenceError.
    let serialized;
    try {
      ...
      if (this.enableLogging) {
        _log.debug(`[LocalStorage] Set ${fullKey}`);
      }
      
      return true;
    } catch (error) {
      _log.error(`[LocalStorage] Failed to set ${fullKey}:`, error);
      
      // Handle quota exceeded
      if (error.name === 'QuotaExceededError') {
        _log.warn('[LocalStorage] Quota exceeded, attempting cleanup');
        this._cleanup();
  ...
  get(key, defaultValue = null) {
    ...
    try {
      ...
      if (this.enableLogging) {
        _log.debug(`[LocalStorage] Get ${fullKey}`);
      }
      
      return parsed.value;
    } catch (error) {
      _log.error(`[LocalStorage] Failed to get ${fullKey}:`, error);
      return defaultValue;
    }
  }
  ...
  remove(key) {
    ...
    try {
      ...
      if (this.enableLogging) {
        _log.debug(`[LocalStorage] Removed ${fullKey}`);
      }
      
      return true;
    } catch (error) {
      _log.error(`[LocalStorage] Failed to remove ${fullKey}:`, error);
      return false;
    }
  }
  ...
  clear() {
    ...
    try {
      ...
      if (this.enableLogging) {
        _log.debug(`[LocalStorage] Cleared ${removed} keys`);
      }
    } catch (error) {
      _log.error('[LocalStorage] Failed to clear:', error);
    }
  ...
  keys() {
    ...
    try {
      ...
      _log.error('[LocalStorage] Failed to list keys:', error);
    }
  ...
  getStats() {
    ...
    try {
      ...
      _log.error('[LocalStorage] Failed to get stats:', error);
    }
  ...
  _cleanup() {
    try {
      ...
      _log.debug(`[LocalStorage] Cleanup removed ${removeCount} old entries`);
    } catch (error) {
      _log.error('[LocalStorage] Cleanup failed:', error);
    }
  }
}
...
if (typeof window !== 'undefined') {
  ...
  _log.debug('LocalStorage loaded');
}
