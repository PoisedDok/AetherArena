...
class IpcBridge {
  constructor(options = {}) {
    ...
    this.log = createLogger({ component: `IpcBridge:${this.context}` });
    ...
    if (!this.ipc) {
      this.log.warn('IPC API not available, queueing messages');
    }
  }
  ...
  send(channel, payload) {
    ...
    try {
      ...
      this.log.debug('sent', { channel });
      
      return true;
    } catch (error) {
      this.log.error(`failed to send on ${channel}`, { error: error.message });
      return false;
    }
  }
  ...
  on(channel, handler) {
    if (!this.ipc) {
      this.log.warn(`cannot register listener for ${channel} - IPC not available`);
      return () => {};
    }
    ...
    try {
      ...
      const wrappedHandler = (...args) => {
        this.log.debug('received', { channel });
        handler(...args);
      };
      ...
      this.log.error(`failed to register listener for ${channel}`, { error: error.message });
      return () => {};
    }
  }
  ...
  once(channel, handler) {
    if (!this.ipc) {
      this.log.warn(`cannot register once listener for ${channel} - IPC not available`);
      return () => {};
    }
    
    try {
      const wrappedHandler = (...args) => {
        this.log.debug('received (once)', { channel });
        handler(...args);
      ...
      this.log.error(`failed to register once listener for ${channel}`, { error: error.message });
      return () => {};
    }
  }
  ...
  removeListener(channel, handler) {
    ...
    try {
      ...
      this.log.error(`failed to remove listener for ${channel}`, { error: error.message });
    }
  }
  ...
  removeAllListeners(channel) {
    ...
    // BUG 11 FIX: spread to avoid concurrent mutation -- removeListener
    // splices the same array, causing the for...of iterator to skip elements.
  ...
  flushQueue() {
    ...
    this.log.debug(`flushed ${count} queued messages`);
  ...
  _queueMessage(channel, payload) {
    ...
    this.log.debug(`queued message for ${channel}`);
  }
}
