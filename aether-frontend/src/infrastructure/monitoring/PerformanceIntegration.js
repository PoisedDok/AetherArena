...
class PerformanceIntegration {
  constructor(options = {}) {
    this.log = createRendererLogger('PerformanceIntegration');
    ...
    if (this.enableLogging) {
      this.log.info('[PerformanceIntegration] Initialized');
    }
  }
  ...
  start() {
    ...
    if (this.enableLogging) {
      this.log.info('[PerformanceIntegration] Started monitoring');
    }
  }
  ...
  stop() {
    ...
    if (this.enableLogging) {
      this.log.info('[PerformanceIntegration] Stopped monitoring');
    }
  }

  /**
   * Dispose all monitoring components and release resources
   */
  dispose() {
    if (this._isDisposed) return;
    this._isDisposed = true;

    if (this.memoryMonitor && typeof this.memoryMonitor.dispose === 'function') {
      this.memoryMonitor.dispose();
    }
    if (this.metricsCollector && typeof this.metricsCollector.dispose === 'function') {
    ...
    if (typeof window !== 'undefined' && window.__PERFORMANCE__ === this) {
      window.__PERFORMANCE__ = null;
    }
  }
  ...
  async completeStartup() {
    ...
    if (this.enableLogging) {
      ...
      if (!startupValidation.passed) {
        this.log.warn('[PerformanceIntegration] Startup budget violations detected');
      } else {
        this.log.info('[PerformanceIntegration] ✅ All startup budgets passed');
      }
    }
  ...
  logStatus() {
    ...
    this.log.info('[PerformanceIntegration] Performance Status', {
      startup: this.startupProfiler.getSummary(),
      budgetOverall: budgetStatus.overall,
      violations: budgetStatus.violations,
      warnings: budgetStatus.warnings,
      performance: perfSummary,
    });

    // Delegate memory status to MemoryMonitor's structured logger
  ...
  _handleMemoryBudgetExceeded(memory) {
    this.log.error('[PerformanceIntegration] ❌ MEMORY BUDGET EXCEEDED:', memory);
  ...
  _handleLeakDetected(leak) {
    this.log.warn('[PerformanceIntegration] ⚠️  POTENTIAL MEMORY LEAK DETECTED:', leak);
  }
  ...
  _handleBudgetViolation(violation) {
    this.log.warn('[PerformanceIntegration] ⚠️  BUDGET VIOLATION:', violation);
  }
  ...
  _handleBudgetWarning(warning) {
    if (this.enableLogging) {
      this.log.warn('[PerformanceIntegration] ⚠️  BUDGET WARNING:', warning);
    }
  }
}
