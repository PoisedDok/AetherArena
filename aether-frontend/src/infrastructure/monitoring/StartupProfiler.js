...
class StartupProfiler {
  constructor(options = {}) {
    this.log = createRendererLogger('StartupProfiler');
    ...
    if (this.enableLogging) {
      this.log.info('[StartupProfiler] Initialized - tracking startup performance');
    }
  }
  ...
  startPhase(phase) {
    if (this.isStartupComplete) {
      this.log.warn('[StartupProfiler] Cannot start phase - startup already complete');
      return;
    }
    ...
    if (this.enableLogging) {
      this.log.info(`[StartupProfiler] ⏱️  Phase started: ${phase}`);
    }
  }
  ...
  endPhase(phase) {
    ...
    if (!phaseData) {
      this.log.warn(`[StartupProfiler] Phase not found: ${phase}`);
      return null;
    }

    if (phaseData.endTime !== null) {
      this.log.warn(`[StartupProfiler] Phase already ended: ${phase}`);
    ...
    if (this.enableLogging) {
      this.log.info(`[StartupProfiler] ✅ Phase completed: ${phase} (${phaseData.duration}ms)`);
    }
  ...
  markMilestone(milestone) {
    ...
    if (this.enableLogging) {
      this.log.info(`[StartupProfiler] 🎯 Milestone: ${milestone} (+${elapsed}ms)`);
    }
  ...
  completeStartup() {
    ...
    if (this.enableLogging) {
      this.log.info(`[StartupProfiler] 🏁 Startup complete: ${totalTime}ms`);
      
      if (totalTime > this.budget) {
        this.log.warn(`[StartupProfiler] ⚠️  Startup exceeded budget: ${totalTime}ms > ${this.budget}ms`);
      } else {
        this.log.info(`[StartupProfiler] ✅ Startup within budget: ${totalTime}ms < ${this.budget}ms`);
      }
    }
  }
  ...
  logSummary() {
    ...
    const lines = [
      `Total: ${summary.total}ms | Budget: ${summary.budget}ms | ${summary.withinBudget ? 'PASS' : 'FAIL'}`,
      summary.overbudget > 0 ? `Over budget by: ${summary.overbudget}ms` : null,
      ...summary.slowestPhases.map(p => `  ${p.name}: ${p.duration}ms`),
      summary.firstPaint ? `FP: ${summary.firstPaint}ms` : null,
      summary.firstContentfulPaint ? `FCP: ${summary.firstContentfulPaint}ms` : null,
    ].filter(Boolean);

    this.log.info('[StartupProfiler] Performance Summary', { details: lines.join(' | ') });
  }
   ...
   */
  _getWebVitals() {
    const fcp = this.milestones.get(MILESTONES.FIRST_CONTENTFUL_PAINT)?.elapsed;
    const tti = this.milestones.get(MILESTONES.INTERACTIVE)?.elapsed;
    return {
      FCP: fcp != null ? fcp : null,
      TTI: tti != null ? tti : null,
    };
  }
}
