 ...
 * Outgoing: Return values (duration/stats/summary/renderTiming/resourceTiming/JSON), structured log.warn for slow/critical operations, window.PerformanceMonitor global --- {number | object | array | string | class_reference, javascript_object | global}
 * 
 * 
...
class PerformanceMonitor {
  constructor(options = {}) {
    this.log = createLogger({ component: 'PerformanceMonitor' });
  ...
  start(name) {
    ...
    if (this.available) {
      try {
        ...
        this.log.error(`failed to mark ${markName}`, { error: error.message });
      }
    }
   ...
   */
    ...
    if (this.available) {
      try {
        ...
        this.log.error(`failed to measure ${name}`, { error: error.message });
      }
    }
    ...
    if (this.enableLogging) {
      this.log.debug(`ended: ${name} (${duration}ms)`);
    }
  ...
  getRenderTiming() {
    ...
    try {
      ...
      this.log.error('failed to get render timing', { error: error.message });
      return null;
    }
  }
  ...
  getResourceTiming() {
    ...
    try {
      ...
      this.log.error('failed to get resource timing', { error: error.message });
      return [];
    }
  }
  ...
  clear() {
    ...
    if (this.available) {
      try {
        ...
        this.log.error('failed to clear', { error: error.message });
      }
    }
    
    if (this.enableLogging) {
      this.log.debug('cleared all measurements');
    }
  }
