...
class MemoryMonitor {
  constructor(options = {}) {
    this.log = createRendererLogger('MemoryMonitor');
    ...
    if (!this.hasMemoryAPI) {
      this.log.warn('[MemoryMonitor] Performance.memory API not available');
    }

    if (this.enableLogging) {
      this.log.info('[MemoryMonitor] Initialized with budget:', this._formatBytes(this.budget));
    }
  }
  ...
  start() {
    ...
    if (!this.hasMemoryAPI) {
      this.log.warn('[MemoryMonitor] Cannot start - Memory API not available');
      return;
    }
    ...
    if (this.enableLogging) {
      this.log.info('[MemoryMonitor] Started monitoring');
    }
  }
  ...
  stop() {
    ...
    if (this.enableLogging) {
      this.log.info('[MemoryMonitor] Stopped monitoring');
    }
  }
  ...
  forceGC() {
    ...
    if (typeof global !== 'undefined' && global.gc) {
      if (this.enableLogging) {
        this.log.info('[MemoryMonitor] Forcing garbage collection');
      }
      try {
        global.gc();
      } catch (error) {
        this.log.warn('[MemoryMonitor] GC failed:', error);
      }
    } else {
      if (this.enableLogging) {
        this.log.info('[MemoryMonitor] GC not available (requires --expose-gc)');
      }
    }
  }
  ...
  logStatus() {
    ...
    if (!stats) {
      this.log.info('[MemoryMonitor] No data available');
      return;
    }

    const lines = [
      `Current: ${this._formatBytes(stats.current.used)} / ${this._formatBytes(this.budget)}`,
      `Peak: ${this._formatBytes(stats.peak)}`,
      `Average: ${this._formatBytes(stats.average)}`,
      `Budget status: ${stats.withinBudget ? 'Within budget' : 'Over budget'}`,
      stats.overBudget > 0 ? `Over budget by: ${this._formatBytes(stats.overBudget)}` : null,
      `Trend: ${this.getTrend()}`,
      `Growth rate: ${this._formatBytes(stats.growthRate)}/min`,
      stats.leaks > 0 ? `Potential leaks detected: ${stats.leaks}` : null,
    ].filter(Boolean);

    this.log.info('[MemoryMonitor] Memory Status', { details: lines.join(' | ') });
  }
  ...
  clear() {
    ...
    if (this.enableLogging) {
      this.log.info('[MemoryMonitor] Cleared history');
    }
  }
    ...
    if (used > this.budget) {
      ...
      if (this.onBudgetExceeded) {
        try {
          ...
          this.log.error('[MemoryMonitor] Budget exceeded callback error:', error);
        }
      }

      if (this.enableLogging) {
        this.log.error(`[MemoryMonitor] BUDGET EXCEEDED: ${this._formatBytes(used)} > ${this._formatBytes(this.budget)}`);
      }
    }
    // Critical level
      ...
      if (this.onCritical) {
        try {
          ...
          this.log.error('[MemoryMonitor] Critical callback error:', error);
        }
      }

      if (this.enableLogging) {
        this.log.warn(`[MemoryMonitor] CRITICAL: ${this._formatBytes(used)}`);
      }
    }
    // Warning level
      ...
      if (this.onWarning) {
        try {
          ...
          this.log.error('[MemoryMonitor] Warning callback error:', error);
        }
      }
    }
  }
    ...
    if (growth > THRESHOLDS.LEAK_THRESHOLD) {
      ...
      if (this.onLeakDetected) {
        try {
          ...
          this.log.error('[MemoryMonitor] Leak detected callback error:', error);
        }
      }

      if (this.enableLogging) {
        this.log.warn(`[MemoryMonitor] POTENTIAL LEAK: Growth of ${this._formatBytes(growth)} over ${recent.length} samples`);
      }
    }
  }
