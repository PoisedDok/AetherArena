...
class AudioStreamService {
  constructor() {
    ...
    this.log = createLogger({ component: 'AudioStreamService' });
  }
  ...
  startStream(streamId, mediaStream, mediaRecorder, audioContext) {
    ...
    // PRODUCTION FIX: MediaRecorder optional (ScriptProcessorNode doesn't use it)
    if (mediaRecorder) {
  ...
  cleanupAllStreams() {
    for (const [streamId, stream] of this._activeStreams.entries()) {
      try {
        ...
        this.log.error('Failed to cleanup stream', { streamId, error: e?.message });
      }
    }
