...
class ArtifactStreamHandler {
  constructor(dependencies = {}) {
    ...
    this.logger = dependencies.logger || createLogger({ component: 'ArtifactStreamHandler' });
  ...
  async handleStreamChunk(streamData) {
    try {
      ...
      if (streamData?.id) {
        const artifactId = Artifact.generateIdWithKind(streamData.id, streamData.kind);
