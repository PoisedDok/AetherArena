...
class SettingsService {
  ...
  async saveSettings(settings) {
    ...
    try {
      // FAIL-FAST: validate settings before persisting/sending to backend.
      // Backend still validates on receive, but frontend must block obviously bad inputs
      // to avoid runtime crashes and security bypasses.
      this.validateSettings(settingsInstance, { throwOnError: true });
   ...
   * @param {boolean} [options.throwOnError=true] - Whether to throw on validation failure
   ...
   * ARCHITECTURE: Backend validates on receive - frontend just parses and proxies
