...
class MessageService {
  ...
  createUserMessage(content, chatId = null, options = {}) {
    this._assertValidContent(content);
  ...
  validateMessage(message) {
    ...
    return this._isValidMessageShape(message);
  }

  _assertValidContent(content) {
    if (typeof content !== 'string' || content.trim().length === 0) {
      throw new Error('[MessageService] Invalid message content');
    }
  }

  _isValidMessageShape(message) {
