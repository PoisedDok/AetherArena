...
async function validateCSP() {
  ...
  try {
    const cspModulePath = path.join(__dirname, '..', 'src', 'core', 'security', 'CspManager.js');
    ...
    // Dynamically import CSP defaults to avoid brittle regex checks
    // eslint-disable-next-line global-require, import/no-dynamic-require
    const { DEFAULT_POLICIES } = require(cspModulePath);
    const prodPolicy = DEFAULT_POLICIES?.production;
    if (!prodPolicy) {
      return {
        passed: false,
        error: 'Production CSP policy not found',
      };
    }
    ...
    for (const [directive, values] of Object.entries(prodPolicy)) {
      ...
      for (const pattern of unsafePatterns) {
        if (values.includes(pattern)) {
          issues.push({
            severity: 'critical',
            category: 'csp',
            issue: `Unsafe directive in production: ${directive} ${pattern}`,
    ...
    for (const directive of requiredDirectives) {
      if (!Object.prototype.hasOwnProperty.call(prodPolicy, directive)) {
        issues.push({
          severity: 'high',
          category: 'csp',
...
async function checkLicenses() {
  ...
  try {
    const { stdout } = await execPromise('npx license-checker --json', {
    ...
    for (const [pkg, info] of Object.entries(licenses)) {
      if (!isLicenseAllowed(info.licenses)) {
        issues.push({
          severity: 'high',
          category: 'license',
          package: pkg,
          license: info.licenses,
          issue: `Unapproved license detected: ${info.licenses}`,
          recommendation: 'Review dependency or update the approved license list',
        });
      }
    }
    ...
    return {
      passed: false,
      error: error.message || 'Failed to execute license-checker',
    };
  }
}

function isLicenseAllowed(value) {
  const tokens = extractLicenseTokens(value);
  if (tokens.length === 0) {
    return false;
  }
  return tokens.every(token => LICENSE_ALLOWLIST.has(token));
}
