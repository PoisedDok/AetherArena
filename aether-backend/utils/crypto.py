...
Processing: encrypt(), decrypt(), hash_password(), generate_token(), encrypt_config() --- {JOB_HASH_SECRET, JOB_MANAGE_SECRETS, JOB_MANAGE_TOKENS}
...
def _crypto():
    ...
    if _CRYPTO_MODULE is None:
        _CRYPTO_MODULE = import_module("security.crypto")
    return _CRYPTO_MODULE


CryptoError = _crypto().CryptoError
...
def encrypt(plaintext: str, key_file: Optional[Path] = None) -> str:
    ...
    """
    manager = _crypto().get_secret_manager(key_file=key_file)
...
def hash_password(password: str, rounds: int = 12) -> str:
    ...
    """
    return _crypto().Hasher.hash_password(password, rounds=rounds)
...
def verify_password(password: str, hashed: str) -> bool:
    ...
    """
    return _crypto().Hasher.verify_password(password, hashed)
...
def generate_token(length: int = 32) -> str:
    ...
    """
    return _crypto().Hasher.generate_token(length)
...
def generate_api_key(prefix: str = "aether", length: int = 32) -> Tuple[str, str]:
    ...
    """
    manager = _crypto().get_secret_manager()
...
def hash_token(token: str) -> str:
    ...
    """
    return _crypto().Hasher.hash_token(token)
...
def checksum_file(file_path: Path, algorithm: str = 'sha256') -> str:
    ...
    """
    return _crypto().Hasher.checksum_file(file_path, algorithm=algorithm)
...
def constant_time_compare(a: str, b: str) -> bool:
    ...
    Example:
        ...
        ...     # Token is valid
    """
    return _crypto().Hasher.constant_time_compare(a, b)
