...
@.architecture
Incoming: backend modules, security/sanitization.py --- {str text, bytes payload}
Processing: delegate to security sanitizer, enforce validation helpers, expose convenience wrappers --- {JOB_SANITIZE}
Outgoing: backend modules --- {str sanitized_value, bool injection_flag}
"""
...
_SANITIZATION_MODULE = None


def _sanitization():
    global _SANITIZATION_MODULE
    if _SANITIZATION_MODULE is None:
        _SANITIZATION_MODULE = import_module("security.sanitization")
    return _SANITIZATION_MODULE


ValidationError = _sanitization().ValidationError
SizeExceededError = _sanitization().SizeExceededError
PathTraversalError = _sanitization().PathTraversalError
InputSanitizer = _sanitization().InputSanitizer
SizeLimits = _sanitization().SizeLimits


def get_sanitizer(*args, **kwargs):
    return _sanitization().get_sanitizer(*args, **kwargs)
