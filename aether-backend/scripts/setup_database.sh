...
if [ "$USER_EXISTS" = "1" ]; then
    ...
    # Securely set the password using a temporary file to avoid shell string interpolation leakage
    TMP_SQL=$(mktemp)
    chmod 600 "$TMP_SQL"
    cat <<EOF > "$TMP_SQL"
...
EOF
    psql -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_ADMIN_USER" -f "$TMP_SQL"
    rm -f "$TMP_SQL"
