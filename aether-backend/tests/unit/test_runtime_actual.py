...
class TestRequestTrackerReal:
    ...
    async def test_request_lifecycle(self):
        ...
        # Start request
        await tracker.start_request(request_id, client_id, text=text)
    ...
    async def test_cancel_request(self):
        ...
        await tracker.start_request(request_id, "client", text="text")
    ...
    async def test_concurrent_requests(self):
        ...
        tasks = [
            tracker.start_request(req_id, f"client-{i}", text=f"text-{i}")
    ...
    async def test_stale_request_cleanup(self):
        ...
        # Start a request
        await tracker.start_request("stale-req", "client", text="text")
    ...
    async def test_async_safety(self):
        ...
        async def worker(i):
            req_id = f"req-{i}"
            await tracker.start_request(req_id, f"client-{i}", text="text")
...
class TestInterpreterManagerReal:
    ...
    async def test_initialization(self):
        """Test interpreter manager can initialize background tasks.
        
        is_initialized() reflects external server *configuration* state,
        not whether initialize() was called.  External server must be
        configured separately via configure_external_server().
        """
        manager = InterpreterManager()
        
        try:
            # External server not yet configured
            assert not manager.is_initialized()
            assert not manager.is_available()
            
            # initialize() starts background cleanup -- always succeeds in external mode
            result = await manager.initialize()
            assert result is True
            
            # is_initialized() still False: external server not configured
            # This is correct -- is_initialized() == _external_server_enabled
            assert not manager.is_initialized()
        finally:
...
class TestDocumentProcessorReal:
    ...
    def test_pipeline_config_pdf(self, processor):
        ...
        assert config["ocr_engine"] == vision_doc.ocr_engine
        assert config["output_format"] == vision_doc.output_format
    
    def test_pipeline_config_image(self, processor):
        """Test pipeline config for image files.
        
        VLM model details are resolved at runtime via resolve_service_provider,
        so the pipeline config only contains pipeline type and output_format.
        """
        settings = get_settings()
        vision_doc = settings.vision_document
...
class TestRuntimeIntegration:
    ...
    async def test_request_tracking_in_streaming(self):
        ...
        await request_tracker.start_request(request_id, "client", text="test")
