...
@.architecture
Incoming: app.py::create_app, exception events from endpoints --- {Request, Exception}
Processing: capture failures, classify outcomes, sanitize payloads, emit structured logs --- {JOB_LOG, JOB_SANITIZE}
Outgoing: monitoring/logging.py, frontend HTTP clients --- {Dict[str, Any], JSONResponse}
"""
    ...
    def _classify_error(self, error: Exception) -> tuple[int, str, str]:
        ...
        try:
            from core.exceptions import (
                NetworkTimeoutError, UpstreamServiceError, NetworkConnectionError,
                InvalidRequestError, ResourceNotFoundError
            )
        except ImportError:
            NetworkTimeoutError = type('NetworkTimeoutError', (Exception,), {})
            UpstreamServiceError = type('UpstreamServiceError', (Exception,), {})
            NetworkConnectionError = type('NetworkConnectionError', (Exception,), {})
            InvalidRequestError = type('InvalidRequestError', (Exception,), {})
            ResourceNotFoundError = type('ResourceNotFoundError', (Exception,), {})
        
        # Map exceptions to status codes
        # NOTE: SizeExceededError and PathTraversalError are subclasses of
        # ValidationError — check them BEFORE the generic ValidationError.
        if isinstance(error, SizeExceededError):
            ...
            return 504, str(error), error_type
        elif isinstance(error, NetworkConnectionError):
            return 503, str(error), error_type
        elif isinstance(error, InvalidRequestError):
            return 400, str(error), error_type
        elif isinstance(error, ResourceNotFoundError):
            return 404, str(error), error_type
        elif isinstance(error, UpstreamServiceError):
            return getattr(error, 'status_code', 502), str(error), error_type
