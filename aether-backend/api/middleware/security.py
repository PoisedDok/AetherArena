...
@.architecture
Incoming: app.py::create_app, HTTP pipeline --- {Request, Response}
Processing: apply security header policies to outbound responses --- {JOB_APPLY_CONFIG}
Outgoing: Frontend HTTP clients --- {Response, http_headers}
"""
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if response is None:
                logger.error("Downstream handler returned None for %s", request.url.path)
                from fastapi.responses import JSONResponse
                response = JSONResponse(
                    status_code=500,
                    content={"detail": "Internal server error: No response from handler"}
                )
            ...
            # Client disconnected before downstream returned a response.
            # Avoid noisy error logs for health probes.
            logger.warning(
                "Client disconnected before response was returned",
            ...
            if "No response returned" in str(e):
                logger.warning(
                    "Downstream returned no response",
                    extra={"path": request.url.path, "method": request.method}
                )
                return Response(status_code=204)
            raise
            ...
            logger.error(
                f"Security middleware caught exception for {request.method} {request.url.path}: {e}",
                exc_info=True
            )
    ...
    if production:
        ...
        config = SecurityHeadersConfig(
            ...
            csp_directives={
                ...
                "base-uri": "'self'",
                "form-action": "'self'",
                "frame-ancestors": "'self' app: aether: file:",  # Allow local electron framing
