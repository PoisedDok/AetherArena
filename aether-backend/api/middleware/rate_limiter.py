...
@.architecture
Incoming: app.py::create_app, security/rate_limit.py, HTTP requests --- {Request, Dict[str, Any]}
Processing: derive client identity, select rate tiers, enforce token buckets, emit limit headers --- {JOB_MANAGE_RATE_LIMITS}
Outgoing: Frontend HTTP responses, security/rate_limit.py --- {Response, Dict[str, Any]}
"""
...
class RateLimiterMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        app: ASGIApp,
        enabled: bool = True,
        tier_overrides: Optional[Sequence[Tuple[str, str]]] = None,
    ):
        """
        ...
        self._tier_overrides = list(tier_overrides or [])
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            logger.error("Error in rate limiter: %s", e, exc_info=True)
            return await call_next(request)
    ...
    def _get_tier_for_path(self, path: str) -> str:
        ...
        """
        # Priority overrides from configuration (glob patterns)
        for pattern, tier in self._tier_overrides:
...
def create_rate_limiter_middleware(
    ...
    return (
        ...
        {"enabled": enabled, "tier_overrides": tier_overrides},
    )
