...
Processing: nb_sys_path_add(), nb_import_module(), nb_list_packages(), nb_search_modules(), nb_get_module_info() --- {JOB_DISCOVER_TOOLS, JOB_EXECUTE_TOOL, JOB_LOAD_CONFIG, JOB_MANAGE_CONNECTION}
...
def nb_sys_path_add(path: str, prepend: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if prepend:
            ...
            logger.debug("Prepended %s to sys.path", normalized_path)
        else:
            # Append if not already present
            if normalized_path not in sys.path:
                sys.path.append(normalized_path)
                logger.debug("Appended %s to sys.path", normalized_path)
...
def nb_import(
    ...
    try:
        ...
        if reload and module in sys.modules:
            ...
            logger.debug("Reloaded module: %s", module)
        else:
        ...
        if alias and add_to_builtins:
            ...
            logger.debug("Bound %s as %s in builtins", module, alias)
            ...
            for name in fromlist:
                try:
                    ...
                    if add_to_builtins:
                        ...
                        logger.debug("Bound %s.%s in builtins", module, name)
...
def nb_import_from_path(
    ...
    try:
        ...
        if reload or module not in sys.modules:
            ...
            logger.debug("Loaded module %s from %s", module, path)
    ...
    try:
        ...
        if method == "metadata":
            try:
                ...
                logger.warning("Metadata method failed, falling back to pkgutil: %s", e)
                method = "pkgutil"
        ...
        logger.debug("Listed %d packages using %s method", len(packages), method)
...
def nb_search_importable(
    ...
    try:
        ...
        logger.debug("Found %d importable modules matching '%s'", len(matches), query)
...
def nb_module_info(module: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("Retrieved info for module: %s", module)
        return info
...
def nb_execute_code(code: str, session_id: Optional[str] = None, timeout: Optional[int] = 30) -> Dict[str, Any]:
    """
    Execute Python code in an isolated namespace and capture output.
    
    Args:
        code: Python code to execute
        session_id: Optional session ID for stateful execution (currently unused)
        timeout: Execution timeout in seconds (currently unused, but reserved for future)
    ...
    try:
        ...
        # Simple code execution in isolated namespace
        exec_globals = {}
        exec_locals = {}
