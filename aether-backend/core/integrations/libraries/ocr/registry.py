...
Processing: get_backend(), register_backend(), list_backends(), create_backend() --- {JOB_DISCOVER_TOOLS, JOB_INITIALIZE_COMPONENT, JOB_MANAGE_CONNECTION, JOB_REGISTER_PROVIDER}
...
class OCRBackendRegistry:
        ...
        **kwargs
        ...
        try:
            ...
            if backend and backend.is_available():
                ...
                logger.info("Created OCR backend: %s", backend_type.value)
                return backend
            else:
                logger.warning("Backend %s not available", backend_type.value)
                return None
                
        except Exception as e:
            logger.error("Failed to create backend %s: %s", backend_type.value, e)
            return None
    ...
    def _create_backend(
        ...
        **kwargs
        ...
        elif backend_type == OCRBackendType.GLM_OCR:
            return GlmOcrBackend(**kwargs)
    ...
    def set_default_backend(cls, backend_type: OCRBackendType) -> None:
        ...
        logger.info("Default OCR backend set to: %s", backend_type.value)
    ...
    def unload_all(cls) -> None:
        ...
        for backend_type, backend in cls._backends.items():
            try:
                ...
                logger.info("Unloaded backend: %s", backend_type.value)
            except Exception as e:
                logger.error("Failed to unload %s: %s", backend_type.value, e)
