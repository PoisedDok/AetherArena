...
Processing: process_image(), process_file(), _load_model(), _run_inference() --- {JOB_EXECUTE_TOOL, JOB_INITIALIZE_COMPONENT, JOB_LOAD_CONFIG, JOB_TRANSFORM_DATA}
...
class PaddleOCRVLIntegration:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        try:
            ...
            logger.info("PaddleOCR-VL integration initialized (device: %s)", self._device)
        except Exception as e:
            logger.warning("PaddleOCR-VL not available: %s", e)
    ...
    def load_model(self, force_reload: bool = False) -> bool:
        ...
        try:
            logger.info("Loading PaddleOCR-VL-0.9B from %s", self._model_path)
            logger.info("Target device: %s", self._device)
            ...
            logger.info("Model loaded (device: %s, memory optimized)", self._device)
    ...
    def process_file(
        ...
        **kwargs
        ...
        try:
            ...
            logger.error("File processing failed: %s", e, exc_info=True)
            return None
