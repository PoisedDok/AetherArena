...
Processing: list_servers(), list_tools(), execute(), health(), _log_execution() --- {JOB_DISCOVER_TOOLS, JOB_EXECUTE_TOOL}
...
class MCPBridge:
    ...
    def install(self) -> bool:
        ...
        try:
            ...
            logger.error("Failed to install MCP bridge: %s", e)
            return False
        ...
        try:
            ...
            for server in servers:
                if not server.get("is_running"):
                    logger.debug("Skipping server %s - not running", server.get("name"))
                    continue
            ...
            logger.info("Dynamically registered %d MCP tools across %d servers", total_tools, len(servers))
            ...
            if has_tool_metadata and tool_metadata_list:
                try:
                    ...
                    if hasattr(self._computer, 'tools') and hasattr(self._computer.tools, '_engine'):
                        ...
                        logger.info("Indexed %d MCP tools in semantic search", len(tool_metadata_list))
                    else:
                        logger.warning("Tool engine not available - MCP tools not indexed for search")
                except Exception as e:
                    logger.error("Failed to register MCP tools in engine: %s", e, exc_info=True)
            ...
            if hasattr(self._computer, 'mcp_tools'):
                try:  
                    ...
                    logger.warning("Failed to populate MCPToolsClass: %s", e)
            
        except Exception as e:
            logger.error("Failed to register dynamic MCP tools: %s", e, exc_info=True)
    ...
    def execute(
        ...
        **arguments
        ...
        try:
            ...
            logger.error("Failed to execute %s on %s: %s", tool_name, server_name, e)
            return {
                "success": False,
                "error": str(e),
                "duration_ms": 0,
    ...
    def health(self, server_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Failed to check health for %s: %s", server_name, e)
            return {
                "healthy": False,
                "status": "error",
                "error": str(e)
...
class MCPToolsClass:
                ...
                for tool in tools:
                    ...
                    def make_tool_method(srv_name, tl_name, tl_desc):
                        def tool_method(**kwargs):
                            """MCP tool method"""
                            return self._bridge.execute(srv_name, tl_name, **kwargs)
                    ...
                    logger.debug("Added MCP tool method: %s", tool_name)
            
            self._tools_populated = True
            logger.info("MCP tools populated in MCPToolsClass")
            
        except Exception as e:
            logger.error("Failed to populate MCP tools: %s", e)
    ...
    def execute(self, server_name: str, tool_name: str, **kwargs):
        """Execute a tool"""
        return self._bridge.execute(server_name, tool_name, **kwargs)
