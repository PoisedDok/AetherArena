...
Processing: check_all(), _check_load(), _check_attach(), _check_execute(), _validate_signature() --- {JOB_HEALTH_CHECK}
...
class IntegrationHealthChecker:
    ...
    def __init__(self, aether_backend_root: Optional[Path] = None):
        ...
        logger.info("IntegrationHealthChecker initialized (aether-backend: %s)", self.aether_backend_root)
    ...
    def check_all(self) -> Dict[str, HealthCheckResult]:
        ...
        for name, config in integrations.items():
            if not config.get("enabled", True):
                ...
                logger.debug("Skipping disabled integration: %s", name)
                continue
    ...
    def check_integration(self, name: str, config: Dict[str, Any]) -> HealthCheckResult:
        ...
        if result.passed:
            logger.info("%s health check passed", name)
        else:
    ...
    def _check_load(
        ...
        try:
            ...
            logger.debug("%s: module loaded successfully (%s)", name, init_module)
            return True, module
            
        except ImportError as e:
            result.errors.append(f"Import failed: {str(e)}")
            logger.debug("%s: import failed - %s", name, e)
            return False, None
        except Exception as e:
            result.errors.append(f"Load error: {str(e)}")
            logger.debug("%s: load error - %s", name, e)
            return False, None
    ...
    def _check_attach(
        ...
        try:
            ...
            if missing_exports:
                ...
                logger.debug("%s: missing exports - %s", name, missing_exports)
                return False
            ...
            logger.debug("%s: attach check passed (%d exports)", name, len(exports))
            return True
            
        except Exception as e:
            result.errors.append(f"Attach check error: {str(e)}")
            logger.debug("%s: attach check error - %s", name, e)
            return False
    ...
    def _check_execute(
        ...
        try:
            ...
            for export_name in exports:
                ...
                if callable(export_obj):
                    try:
                        ...
                        executable_count += 1
                        logger.debug("%s.%s: signature OK - %s", name, export_name, sig)
                ...
                # NOTE: Classes are callable (metaclass provides __call__), so they
                # are handled by the `if callable(export_obj)` branch above.
                # No separate isinstance(export_obj, type) check is needed.
            ...
            if len(exports) > 0 and executable_count == 0:
                ...
                logger.debug("%s: no executable exports", name)
                return False
            
            logger.debug("%s: execute check passed (%d/%d inspectable)", name, executable_count, len(exports))
            return True
            
        except Exception as e:
            result.errors.append(f"Execute check error: {str(e)}")
            logger.debug("%s: execute check error - %s", name, e)
            return False
