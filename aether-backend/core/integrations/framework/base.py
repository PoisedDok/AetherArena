...
Processing: load(), health_check(), get_metadata(), cleanup(), _attach_function(), _register_tool() --- {JOB_CLEANUP_RESOURCE, JOB_HEALTH_CHECK, JOB_INITIALIZE_COMPONENT, JOB_ORCHESTRATE, JOB_REGISTER_PROVIDER}
...
class BaseIntegration(ABC):
    ...
    def load(self, computer: Any) -> bool:
        ...
        if self.status == IntegrationStatus.LOADED:
            self.logger.info("%s already loaded", self.name)
            return True
        
        self.status = IntegrationStatus.LOADING
        self.logger.info("Loading %s integration...", self.name)
        ...
        try:
            ...
            if success:
                ...
                self.logger.info("✅ %s loaded successfully", self.name)
                return True
            else:
                self.status = IntegrationStatus.FAILED
                self._load_error = "Load method returned False"
                self.logger.error("❌ %s load failed", self.name)
                return False
                
        except Exception as e:  # noqa: BLE001 -- base class load boundary: must always return bool; subclass _do_load can raise anything
            self.status = IntegrationStatus.FAILED
            self._load_error = str(e)
            self.logger.error("❌ %s load error: %s", self.name, e, exc_info=True)
            return False
    ...
    def health_check(self) -> IntegrationHealth:
        ...
        except Exception as e:  # noqa: BLE001 -- health check boundary: must always return IntegrationHealth; subclass _do_health_check can raise anything
            self.logger.error("Health check error: %s", e)
            return IntegrationHealth(
                healthy=False, message="Health check failed: %s" % type(e).__name__
            )
    ...
    def get_metadata(self) -> IntegrationMetadata:
        ...
        except Exception as e:  # noqa: BLE001 -- metadata boundary: must always return IntegrationMetadata; subclass _enrich_metadata can raise anything
            self.logger.error("Metadata retrieval error: %s", e)
            return IntegrationMetadata(
                name=self.name, version=self.version, description="Error: %s" % type(e).__name__
            )
    ...
    def cleanup(self) -> None:
        ...
        try:
            self._do_cleanup()
            self.logger.info("%s cleanup complete", self.name)
        except Exception as e:  # noqa: BLE001 -- cleanup boundary: must not raise during teardown; subclass _do_cleanup can raise anything
            self.logger.error("Cleanup error for %s: %s", self.name, e)
    ...
    def _attach_function(self, name: str, func: Any) -> None:
        ...
        self.logger.debug("Attached function: %s", name)
    ...
    def _attach_namespace(self, namespace: str, obj: Any) -> None:
        ...
        self.logger.debug("Attached namespace: %s", namespace)
    ...
    def _validate_dependencies(self, dependencies: List[str]) -> Dict[str, bool]:
        ...
        for dep in dependencies:
            ...
            if not available:
                self.logger.warning("Dependency not available: %s", dep)
...
class ServiceIntegration(BaseIntegration):
    ...
    def _do_health_check(self) -> IntegrationHealth:
        ...
        try:
            ...
            return IntegrationHealth(
                healthy=False,
                message="Health check failed: %s" % type(e).__name__,
                details={"url": self.health_check_url, "error": type(e).__name__},
            )
