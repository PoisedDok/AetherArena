...
@.architecture
Incoming: /Volumes/Disk-D/Aether/Aether/AetherArena/aether-backend/core/runtime/engine.py::RuntimeEngine, /Volumes/Disk-D/Aether/Aether/AetherArena/aether-backend/core/runtime/streaming.py::ChatStreamer --- {Tuple[str, str], runtime_control}
Processing: track request lifecycle, manage cancellation flags, record activity timestamps, clean up stale sessions --- {JOB_CLEANUP_RESOURCE, JOB_MANAGE_TASK}
Outgoing: /Volumes/Disk-D/Aether/Aether/AetherArena/aether-backend/core/runtime/streaming.py::ChatStreamer, /Volumes/Disk-D/Aether/Aether/AetherArena/aether-backend/core/runtime/engine.py::RuntimeEngine --- {Dict[str, Any], runtime_signal}
"""
...
class RequestTracker:
    ...
    async def start_request(
        ...
        *,
        ...
        logger.debug("Started tracking request %s for client %s", request_id, client_id)

    async def get_status(self, request_id: str) -> Optional[Dict[str, Any]]:
        """
        Retrieve request status snapshot.
        """
        async with self._lock:
            info = self._active_requests.get(request_id)
            if info is None:
                return None
            return info.copy()
    ...
    async def complete_request(
        ...
        *,
        ...
        async with self._lock:
            info = self._active_requests.get(request_id)
            if info is None:
                logger.debug("Attempted to complete unknown request %s", request_id)
                return
            info["state"] = "completed"
            info["cancelled"] = False
            info["result"] = result
            info["completed_at"] = time.time()
            info["last_activity"] = info["completed_at"]
    ...
    async def fail_request(
        ...
        *,
        ...
        async with self._lock:
            info = self._active_requests.get(request_id)
            if info is None:
                logger.debug("Attempted to fail unknown request %s", request_id)
                return
            info["state"] = "failed"
            info["cancelled"] = False
            info["error"] = error
    ...
    async def request_context(
        ...
        *,
        ...
        """
        Async context manager that tracks the lifecycle of a request.
        """
        start = time.perf_counter()
        await self.start_request(request_id, client_id=client_id, text=text)
        try:
            yield
        except asyncio.CancelledError:
            # Re-raise cancellation immediately - never catch it
            raise
        except Exception as exc:  # noqa: BLE001 -- defensive boundary for arbitrary user code via yield
            await self.fail_request(request_id, error=str(exc))
            raise
        else:
            ...
            if timeout is not None and elapsed > timeout:
                await self.fail_request(
                    request_id,
                    error=f"Request exceeded timeout of {timeout} seconds",
                )
                raise TimeoutError(
                    f"Request {request_id} exceeded timeout of {timeout} seconds"
                )
    ...
    async def cancel_request(self, request_id: str) -> bool:
        ...
        async with self._lock:  # BUG FIX: Lock dict mutation
            ...
            if info is not None:
                ...
                info["completed_at"] = time.time()
                info["last_activity"] = info["completed_at"]
                logger.debug("Marked request %s as cancelled", request_id)
                return True
            logger.debug("Request %s not found in active requests", request_id)
            return False
    ...
    async def end_request(self, request_id: str) -> None:
        ...
        async with self._lock:  # BUG FIX: Lock dict mutation
            if request_id in self._active_requests:
                ...
                logger.debug("Ended tracking request %s", request_id)
        ...
        async with self._lock:  # BUG FIX: Lock dict mutations
            for request_id in stale_requests:
                if request_id in self._active_requests:  # Double-check still exists
                    del self._active_requests[request_id]
                    logger.debug("Cleaned up stale request %s", request_id)
        
        if stale_requests:
            logger.info("Cleaned up %d stale requests", len(stale_requests))
    ...
    async def cancel_all_requests(self) -> int:
        ...
        if count > 0:
            logger.info("Cancelled all %d active requests", count)
