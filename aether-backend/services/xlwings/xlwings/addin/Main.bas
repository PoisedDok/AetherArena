...
Public Function RunPython(PythonCommand As String)
    ...
    ' Embedded code is a PRO-only feature. AetherArena does not ship xlwings PRO.
    If uses_embedded_code = True Then
        MsgBox "Embedded code requires xlwings PRO, which is not shipped with AetherArena."
        Exit Function
    End If
