...
class UdfExecutor:
    ...
    def executor(self):
        if self._executor is None:
            with self._lock:
                if self._executor is None:
                    self._executor = concurrent.futures.ThreadPoolExecutor(initializer=pythoncom.CoInitialize)
...
_udf_executor_manager = UdfExecutor()


def shutdown_udf_executor():
    _udf_executor_manager.shutdown()
...
async def async_thread(base, my_has_dynamic_array, func, args, cache_key, expand):
    try:
        ...
        cache[cache_key] = await loop.run_in_executor(
            _udf_executor_manager.executor, functools.partial(func, *args)
        )
...
async def async_thread_nocaller(
    ...
    try:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(_udf_executor_manager.executor, functools.partial(func, *args))
    ...
    async def _com(self, fn, *args, backoff=1):
        ...
        try:
            return await loop.run_in_executor(
                _udf_executor_manager.executor, functools.partial(fn, copy.copy(self), *args)
            )
        except AttributeError:
            # the Dispatch object that the `_udf_executor_manager.executor` thread
