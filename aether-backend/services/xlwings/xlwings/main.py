...
class App:
    ...
    def render_template(self, template=None, output=None, book_settings=None, **data):
        ...
        try:
            ...
            raise ImportError(
                "xlwings App.render_template() requires xlwings PRO, which is not shipped with AetherArena."
            ) from exc
...
class Sheet:
    ...
    def render_template(self, **data):
        ...
        try:
            ...
            raise ImportError(
                "xlwings Sheet.render_template() requires xlwings PRO, which is not shipped with AetherArena."
            ) from exc
    ...
    def text(self, value):
        if xlwings.__pro__:
            try:
                ...
                # Defensive fallback: if __pro__ is set but PRO isn't actually present,
                # don't crash; just set plain text.
