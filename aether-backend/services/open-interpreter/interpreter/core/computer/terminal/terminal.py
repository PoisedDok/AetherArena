...
import datetime
import sys
oi_path = os.environ.get("OPEN_INTERPRETER_PATH")
if oi_path and oi_path not in sys.path:
    sys.path.insert(0, oi_path)
from interpreter import interpreter

computer = interpreter.computer

# Ensure Notebook helper is attached in runtime sessions
try:
    import importlib
    # Add backend to path for integrations import
    backend_path_str = str(__file__).replace('/open-interpreter/interpreter/core/computer/terminal/terminal.py', '')
    if backend_path_str not in sys.path:
        sys.path.insert(0, backend_path_str)
    
    # Import from new location
    from integrations.notebook.runtime import (
        nb_sys_path_add, nb_import, nb_import_from_path,
        nb_list_sys_path, nb_list_installed, nb_search_importable, nb_module_info
    )
    
    if not hasattr(computer, 'Notebook'):
        computer.Notebook = type('Notebook', (), {
            'sys_path_add': nb_sys_path_add,
            'import_module': nb_import,
            'import_from_path': nb_import_from_path,
            'list_sys_path': nb_list_sys_path,
            'list_installed': nb_list_installed,
            'search_importable': nb_search_importable,
            'module_info': nb_module_info,
        })()
    # Also attach to builtins if present
    try:
        import builtins as _blt
        _blt.computer = computer
        _blt.computer.Notebook = computer.Notebook
    except Exception:
        pass
except Exception:
    pass
...
class Terminal:
    ...
    def run(self, language, code=None, stream=False, display=False):
        """
        Execute code in a given language environment.

        Backward-compatible signature change: if called with a single string
        argument (the command) instead of (language, code), we treat it as a
        shell command. This allows usages like `computer.terminal.run("ls -la")`.
        """
        # Allow single-argument usage where the first arg is actually the code
        if code is None and isinstance(language, str):
            code = language
            language = "shell"
