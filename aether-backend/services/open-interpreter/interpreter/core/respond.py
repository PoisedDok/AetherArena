...
def respond(interpreter):
    ...
    while True:
        ...
        # Only append if non-empty and not already included (respect profile overrides)
        if interpreter.computer.import_computer_api:
            if (interpreter.computer.system_message and 
                interpreter.computer.system_message not in system_message):
                system_message = (
        ...
        if interpreter.messages[-1]["type"] == "code":
            ...
            try:
                ...
                try:
                    from .computer.code_validator import validate_code, get_safe_alternative
                    is_safe, error_msg = validate_code(code, language)
                    if not is_safe:
                        # Block dangerous code and provide helpful feedback
                        yield {
                            "type": "console",
                            "format": "output",
                            "content": f"{error_msg}\n\n{get_safe_alternative(code)}"
                        }
                        # Add error to message history so LLM learns
                        interpreter.messages.append({
                            "type": "console",
                            "format": "output",
                            "content": error_msg
                        })
                        continue  # Skip code execution
                except ImportError:
                ...
                try:
                    ...
                    return  # Use return instead of break to properly handle GeneratorExit
