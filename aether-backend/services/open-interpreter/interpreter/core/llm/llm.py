...
os.environ["LITELLM_LOG"] = "ERROR"  # Suppress LiteLLM debug noise
import sys

import litellm

litellm.suppress_debug_info = True
litellm.set_verbose = False  # Disable verbose logging completely
...
# Silence LiteLLM loggers immediately
...
try:
    ...
    config_path = Path(__file__).parent.parent.parent.parent.parent / "backend" / "config_loader.py"
    if config_path.exists():
        ...
        get_llm_settings = config_loader.get_llm_settings
    else:
        # Fallback if config loader not found
        def load_config():
            return {
                "MODELS": {
...
class Llm:
    ...
    def __init__(self, interpreter):
        ...
        # Settings - use LM Studio configuration
        self.model = llm_settings["model"]  # Should be "qwen/qwen3-4b-2507"
        self.temperature = 0.0

        # Set API configuration for LM Studio
        self.api_base = llm_settings["api_base"]  # Should be "http://localhost:1234/v1"
        self.api_key = config["PROVIDERS"]["lm_studio_api_key"]  # Should be "not-needed"

        self.supports_vision = llm_settings["supports_vision"]  # Should be True
        self.vision_renderer = (
            self.interpreter.computer.vision.query
        )  # Will only use if supports_vision is False

        self.supports_functions = config["OPEN_INTERPRETER"]["supports_functions"]  # Should be False
        ...
        # Optional settings from config
        self.context_window = llm_settings["context_window"]  # Should be 100000
        self.max_tokens = llm_settings["max_tokens"]  # Should be 4096
...
Continuing...
        ...
        # LiteLLM verbose logging disabled - too noisy
        # if self.interpreter.verbose:
