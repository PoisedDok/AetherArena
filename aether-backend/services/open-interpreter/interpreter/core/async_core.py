...
class AsyncInterpreter(OpenInterpreter):
    ...
    def display_message(self, message):
        """
        Override display_message to handle chunk dictionaries for web interface.
        If message is a dict (chunk), send it to output queue.
        Otherwise, fall back to parent implementation.
        """
        if isinstance(message, dict):
            # This is a chunk dictionary - send it to the output queue for web interface
            if self.output_queue:
                try:
                    # Send the chunk to the output queue
                    self.output_queue.sync_q.put(message)
                except Exception as e:
                    print(f"Failed to send chunk to output queue: {e}")
                    # Fall back to parent implementation
                    super().display_message(message)
            else:
                # No output queue, fall back to parent
                super().display_message(message)
        else:
            # This is a regular message string, use parent implementation
            super().display_message(message)
