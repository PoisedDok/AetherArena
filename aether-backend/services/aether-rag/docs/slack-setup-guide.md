...
This guide provides step-by-step instructions for setting up Slack integration with Aether-RAG.

## Overview

Aether-RAG's Slack integration uses MCP (Model Context Protocol) servers to fetch and index your Slack messages for RAG (Retrieval-Augmented Generation). This allows you to search through your Slack conversations using natural language queries.
...
3. **Aether-RAG**: Ensure you have Aether-RAG installed and working
...
4. Enter your app name (e.g., "Aether-RAG Slack Integration")
...
4. Enter a name (e.g., "Aether-RAG Integration")
...
python test_channel_by_id_or_name.py \
  ...
  --workspace-name "Sky Lab Computing" \
  --query "What is Aether-RAG about?"
```
...
**Solution**: This is a common timing issue. The Aether-RAG integration now includes automatic retry logic:
...
3. **Keep MCP Server Running**: Start the MCP server separately and keep it running:
   ...
   # Terminal 2: Run Aether-RAG (it will connect to the running server)
...
5. **Community Support**: Reach out to the Aether-RAG community for help
