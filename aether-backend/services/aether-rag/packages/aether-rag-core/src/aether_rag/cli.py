...
class AetherRagCLI:
    def __init__(self):
        # Always use project-local .aether-rag directory (like .git)
        self.indexes_dir = Path.cwd() / ".aether-rag" / "indexes"
    ...
    def get_index_path(self, index_name: str) -> str:
        ...
        return str(index_dir / "documents.aether-rag")

    def index_exists(self, index_name: str) -> bool:
        index_dir = self.indexes_dir / index_name
        meta_file = index_dir / "documents.aether-rag.meta.json"
        return meta_file.exists()

    def create_parser(self) -> argparse.ArgumentParser:
        parser = argparse.ArgumentParser(
            prog="aether-rag",
            description="The smallest vector index in the world. RAG Everything with Aether-RAG!",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog="""
Examples:
  aether-rag build my-docs --docs ./documents                                  # Build index from directory
  aether-rag build my-code --docs ./src ./tests ./config                      # Build index from multiple directories
  aether-rag build my-files --docs ./file1.py ./file2.txt ./docs/             # Build index from files and directories
  aether-rag build my-mixed --docs ./readme.md ./src/ ./config.json           # Build index from mixed files/dirs
  aether-rag build my-ppts --docs ./ --file-types .pptx,.pdf                  # Index only PowerPoint and PDF files
  aether-rag search my-docs "query"                                           # Search in my-docs index
  aether-rag ask my-docs "question"                                           # Ask my-docs index
  aether-rag list                                                             # List all stored indexes
  aether-rag remove my-docs                                                   # Remove an index (local first, then global)
            """,
        )
        ...
        ask_parser.add_argument(
            ...
            type=str,
            default=None,
            help="Override Ollama-compatible host (defaults to AETHER_RAG_OLLAMA_HOST/OLLAMA_HOST)",
        )
        ask_parser.add_argument(
    ...
    def list_indexes(self):
        # Get all project directories with .aether-rag
        global_registry = Path.home() / ".aether-rag" / "projects.json"
        all_projects = []
        ...
        # Filter to only existing directories with .aether-rag
        valid_projects = []
        for project_dir in all_projects:
            project_path = Path(project_dir)
            if project_path.exists() and (project_path / ".aether-rag" / "indexes").exists():
                valid_projects.append(project_path)

        # Add current project if it has .aether-rag but not in registry
        current_path = Path.cwd()
        if (current_path / ".aether-rag" / "indexes").exists() and current_path not in valid_projects:
        ...
        if total_indexes == 0:
            print("💡 Get started:")
            print("   aether-rag build my-docs --docs ./documents")
        else:
            ...
            if current_indexes_count > 0:
                ...
                current_indexes_dir = current_path / ".aether-rag" / "indexes"
                if current_indexes_dir.exists():
                    ...
                    if current_index_dirs:
                        ...
                        print(f'   aether-rag search {example_name} "your query"')
                        print(f"   aether-rag ask {example_name} --interactive")
            else:
    ...
    def _discover_indexes_in_project(
        ...
        # 1. CLI format: .aether-rag/indexes/index_name/
        cli_indexes_dir = project_path / ".aether-rag" / "indexes"
        ...
        # 2. Apps format: *.aether-rag.meta.json files anywhere in the project
        cli_indexes_dir = project_path / ".aether-rag" / "indexes"
        for meta_file in project_path.rglob("*.aether-rag.meta.json"):
            if meta_file.is_file():
                # Skip CLI-built indexes (which store meta under .aether-rag/indexes/<name>/)
                try:
                ...
                file_base = meta_file.name.replace(".aether-rag.meta.json", "")

                # Apps indexes are considered complete if the .aether-rag.meta.json file exists
                status = "✅"
                ...
                try:
                    ...
                    for related_file in index_dir.glob(f"{file_base}.aether-rag*"):
    ...
    def _find_all_matching_indexes(self, index_name: str):
        ...
        for project_dir in all_projects:
            ...
            # 1) CLI-format index under .aether-rag/indexes/<name>
            index_dir = project_path / ".aether-rag" / "indexes" / index_name
            if index_dir.exists():
            ...
            # 2a) by file base
            for meta_file in project_path.rglob(f"{index_name}.aether-rag.meta.json"):
                if meta_file.is_file():
                    # Skip CLI-built indexes' meta under .aether-rag/indexes
                    try:
                        cli_indexes_dir = project_path / ".aether-rag" / "indexes"
                        if cli_indexes_dir.exists() and cli_indexes_dir in meta_file.parents:
                            continue
    ...
    def _remove_single_match(self, match, index_name: str, force: bool):
        ...
        if kind == "cli":
            print(f"   📍 Path: {project_path / '.aether-rag' / 'indexes' / index_name}")
        else:
    ...
    def _remove_from_multiple_matches(self, matches, index_name: str, force: bool):
        ...
        for i, match in enumerate(matches, 1):
            ...
            # Show path details
            if kind == "cli":
                print(f"      📍 {project_path / '.aether-rag' / 'indexes' / index_name}")
            else:
            ...
            try:
                if kind == "cli":
                    ...
                    if file_base:
                        size_mb = sum(
                            f.stat().st_size
                            for f in match["files_dir"].glob(f"{file_base}.aether-rag*")
                            if f.is_file()
                        ) / (1024 * 1024)
    ...
    def _delete_index_directory(
        ...
        try:
            if is_app:
                ...
                for f in index_dir.glob(f"{pattern_base}.aether-rag*"):
                    try:
                        f.unlink()
                        removed += 1
                    except Exception:
    ...
    async def search_documents(self, args):
        ...
        if self.index_exists(index_name):
            ...
            if not all_matches:
                print(
                    f"Index '{index_name}' not found. Use 'aether-rag build {index_name} --docs <dir> [<dir2> ...]' to create it."
                )
                return
                ...
                match = all_matches[0]
                if match["kind"] == "cli":
                    index_path = str(match["index_dir"] / "documents.aether-rag")
                else:
                    # App format: use the meta file to construct the path
                    meta_file = match["meta_file"]
                    file_base = match["file_base"]
                    index_path = str(meta_file.parent / f"{file_base}.aether-rag")
    ...
    async def ask_questions(self, args):
        ...
        chat = AetherRagChat(index_path=index_path, llm_config=llm_config)
