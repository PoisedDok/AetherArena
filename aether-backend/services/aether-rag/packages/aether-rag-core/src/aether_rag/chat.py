#!/usr/bin/env python3
"""
This file contains the chat generation logic for the Aether-RAG project,
...
def check_ollama_models(host: str) -> list[str]:
    ...
    try:
        ...
        logger.debug("Failed to check Ollama models: %s", e)
        return []
    except Exception as e:
        logger.exception("Unexpected error checking Ollama models: %s", e)
        return []

async def check_ollama_models_async(host: str) -> list[str]:
    return await asyncio.to_thread(check_ollama_models, host)
...
def check_ollama_model_exists_remotely(model_name: str) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        logger.debug("Failed to check remote Ollama model library: %s", e)
    except Exception as e:
        logger.exception("Unexpected error checking remote Ollama model: %s", e)

    # If scraping fails, assume model might exist (don't block user)
    return True, []

async def check_ollama_model_exists_remotely_async(model_name: str) -> tuple[bool, list[str]]:
    return await asyncio.to_thread(check_ollama_model_exists_remotely, model_name)
...
def check_hf_model_exists(model_name: str) -> bool:
    ...
    try:
        ...
        logger.debug("Failed to check HF model existence: %s", e)
        return False
...
def get_popular_hf_models() -> list[str]:
    ...
    try:
        ...
        logger.debug("Failed to fetch popular HF models: %s", e)
...
def search_hf_models_fuzzy(query: str, limit: int = 10) -> list[str]:
    ...
    try:
        ...
        if len(model_names) < 3:
            ...
            for var in variations[:2]:  # Limit to 2 variations to avoid too many API calls
                try:
                    ...
                    logger.debug("Failed to search variation '%s': %s", var, e)
                    continue
        ...
        logger.debug("HF model search failed: %s", e)
        return []
...
class LLMInterface(ABC):
    ...
    async def ask_async(self, prompt: str, **kwargs) -> str:
        """Asynchronous wrapper for ask() to prevent event loop blocking."""
        import asyncio
        return await asyncio.to_thread(self.ask, prompt, **kwargs)
...
class HFChat(LLMInterface):
    ...
    def ask(self, prompt: str, **kwargs) -> str:
        logger.debug("kwargs in HF: %s", kwargs)
...
class SimulatedChat(LLMInterface):
    ...
    def ask(self, prompt: str, **kwargs) -> str:
        ...
        logger.debug("Prompt sent to LLM (simulation): %s...", prompt[:500])
