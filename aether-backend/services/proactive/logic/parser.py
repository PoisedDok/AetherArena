...
def run_pipeline(project_name: str) -> Path:
    ...
    logger.info("Processed log: %s", processed_log_path.name)
    logger.info("Screenshots dir: %s", Path(screenshots_dir).name)
    logger.info("Processed log (+screens): %s", log_sc.name)
    logger.info("Trace: %s", out_trace.name)
    return out_trace
