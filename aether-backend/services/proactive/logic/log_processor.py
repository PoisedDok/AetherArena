...
class LogProcessor:
    ...
    def timestamp_to_seconds(self, timestamp_str):
        ...
        try:
            ...
            logger.error("Error parsing timestamp: %s", timestamp_str)
            return None
        ...
        with open(log_file_path, "r", encoding="utf-8") as f:
            ...
            for line in lines:
                ...
                try:
                    ...
                    logger.warning("JSON parsing error: %s", line)
                    continue
                except Exception as e:
                    logger.warning("Processing error: %s | Error: %s", line, e)
                    continue
    ...
    def process_log_file(self, log_file_path, output_path=None, time_threshold=5.0):
        ...
        logger.info("After double-click cleanup: %d actions", len(actions) - 1)

        # Step 8: Remove single-click directly preceding a nearby drag
        actions = self.cleanup_click_before_drag(actions, tol=5)
        if actions:
            actions.pop()
        logger.info("After click-before-drag and final recording cleanup: %d actions", len(actions) - 1)
        ...
        if output_path:
            ...
            logger.info("Processed log saved to: %s", output_path)
...
Examples:
  ...
  python log_processor.py -p qatar_booking -o result.json -t 3.5
    ...
    if not os.path.exists(input_dir):
        logger.error("Project directory not found: %s", input_dir)
        logger.error("Please ensure the project '%s' exists and has an inputs folder.", args.project)
        return 1
    ...
    if not log_files:
        ...
        logger.error("Supported formats: .txt, .log, .json")
        return 1
    
    if len(log_files) > 1:
        logger.error("Multiple log files found in %s", input_dir)
        logger.error("Found files:")
        for f in log_files:
            logger.error("  %s", os.path.basename(f))
        logger.error("Please ensure only one log file exists in the inputs directory.")
        return 1
    ...
    try:
        ...
        if args.verbose:
            ...
            logger.info("Typing delay threshold: %ss", args.typing_delay)
            logger.info("-" * 50)
        ...
        logger.info("Total actions: %d", len(processed_actions))
        ...
        if args.verbose:
            ...
            for action_type, count in sorted(action_types.items()):
                logger.info("  %s: %d", action_type, count)
        
        return 0
        
    except Exception as e:
        logger.error("Error processing log: %s", e, exc_info=args.verbose)
        return 1
