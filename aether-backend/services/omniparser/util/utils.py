...
# Configure pytesseract (optional - uses system tesseract installation)


def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip2-opt-2.7b", device=None):
    if not device:
        # Allow forcing MLX via env var
        if os.getenv("OMNIPARSER_USE_MLX", "0") == "1":
            device = "mlx"
        else:
    ...
    if model_name == "blip2":
        ...
        if device == 'mlx':
            # Load via the MLX backend provided by transformers>=4.41
...
def get_parsed_content_icon(filtered_boxes, starting_idx, image_source, caption_model_processor, prompt=None, batch_size=128):
    ...
    parsed_content_icon = []
    proc_device = caption_model_processor.get('device', 'cpu')
    processor = caption_model_processor['processor']
    model = caption_model_processor['model']

    for i in range(0, len(non_ocr_boxes), batch_size):
        batch_boxes = non_ocr_boxes[i:i+batch_size]
        batch_imgs = [to_pil(image_source[int(box[1]*image_source.shape[0]):int(box[3]*image_source.shape[0]), int(box[0]*image_source.shape[1]):int(box[2]*image_source.shape[1])]) for box in batch_boxes]
        if proc_device == 'mlx':
            inputs = processor(images=batch_imgs, return_tensors="np")
            outputs = model.generate(**inputs, max_new_tokens=10)
            batch_caps = processor.batch_decode(outputs, skip_special_tokens=True)
        else:
            inputs = processor(images=batch_imgs, return_tensors="pt").to(proc_device)
            outputs = model.generate(**inputs, max_new_tokens=10)
        batch_caps = processor.batch_decode(outputs, skip_special_tokens=True)
        parsed_content_icon.extend(batch_caps)
    ...
    for i, box1_elem in enumerate(boxes):
        ...
        if is_valid_box:
            if ocr_bbox:
                ...
                # Append the full element dict, not just raw bbox list
...
def check_ocr_box(image_source: Union[str, Image.Image], display_img = True, output_bb_format='xywh', goal_filtering=None, easyocr_args=None, use_paddleocr=False):
    ...
    text, coord = perform_apple_vision_ocr(image_source)

    # Apply confidence threshold if specified in easyocr_args
    if easyocr_args and 'text_threshold' in easyocr_args:
        threshold = easyocr_args['text_threshold']
        # Filter based on confidence (Apple Vision confidence is 0-1)
        filtered_text = []
        filtered_coord = []
        for i, txt in enumerate(text):
            # For now, we'll keep all detections since Apple Vision typically has high confidence
            # In a more sophisticated implementation, we could filter by confidence
...
def perform_apple_vision_ocr(image: Image.Image):
    """
    Perform OCR using Apple's Vision framework via PyObjC.

    Args:
        image: PIL Image object

    Returns:
        tuple: (text_list, coord_list) where coord_list contains bounding boxes as [[x,y], [x+w,y], [x+w,y+h], [x,y+h]]
    """
    ...
    data_provider = CGDataProviderCreateWithData(None, image_bytes, len(image_bytes), None)
    ...
    for observation in observations:
        ...
        if len(candidates) > 0:
            recognized_text = str(candidates[0].string())
            if recognized_text.strip():
                ...
                y = (1.0 - bounding_box.origin.y - bounding_box.size.height) * height
                w = bounding_box.size.width * width
                h = bounding_box.size.height * height

                # Convert to the format expected by OmniParser (4 corner points)
                coord.append([[x, y], [x + w, y], [x + w, y + h], [x, y + h]])
