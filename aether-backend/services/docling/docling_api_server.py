...
class ConversionRequest(BaseModel):
    ...
    lm_studio_url: str = "http://127.0.0.1:7090/v1/chat/completions"  # Aether Inference (default); LM Studio fallback: http://localhost:1234/v1/chat/completions
...
def clean_vlm_output(content: str, model: str) -> str:
    ...
    # Apply DocumentUtility extractive pipeline. Small output passes through
    # the gate in full (zero loss); large output gets sentence-level LexRank extraction.
    try:
        from utils.document_processing import DocumentUtility
        util = DocumentUtility(max_context_tokens=10_000)
        result = util.extract_from_text(content, "vlm_output")
        if result:
            content = result
    except Exception as e:
        logger.warning("DocumentUtility extraction failed, keeping full content: %s", e)
...
async def process_excel_file(content: bytes, filename: str, request: ConversionRequest) -> ConversionResponse:
    ...
    try:
        ...
        logger.error(f"Excel processing failed: {e!s}")
        processing_time = time.time() - start_time
        return ConversionResponse(
            success=False,
            content="",
            ...
            engine_used="pandas_excel",
            pages_processed=0,
            error=f"Excel processing failed: {e!s}"
        )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Cannot decode file {filename}: {decode_err!s}")
        ...
        logger.error(f"Code file processing failed: {e!s}")
        processing_time = time.time() - start_time
        return ConversionResponse(
            success=False,
            content="",
            ...
            engine_used="text_processor",
            pages_processed=0,
            error=f"Code processing failed: {e!s}"
        )
...
async def process_text_file(content: bytes, filename: str, request: ConversionRequest) -> ConversionResponse:
    ...
    try:
        ...
        logger.error(f"Text file processing failed: {e!s}")
        processing_time = time.time() - start_time
        return ConversionResponse(
            success=False,
            content="",
            ...
            engine_used="text_processor",
            pages_processed=0,
            error=f"Text processing failed: {e!s}"
        )
...
async def get_options():
    ...
    return OptionsResponse(
        ...
        defaults={
            "lm_studio_url": "http://127.0.0.1:7090/v1/chat/completions",
            "lm_studio_model": "smoldocling-256m-preview-mlx",
...
async def convert_file(
    ...
    lm_studio_url: str = Form("http://127.0.0.1:7090/v1/chat/completions"),
...
async def convert_file_json(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid options JSON: {e!s}")
