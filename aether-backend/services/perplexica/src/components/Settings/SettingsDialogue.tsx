...
const SettingsDialogue = ({
  ...
  return (
    ...
    >
      ...
      >
        <DialogPanel className="space-y-4 border border-light-200 dark:border-dark-200 glass-surface backdrop-blur-lg rounded-xl h-[calc(100vh-2%)] w-[calc(100vw-2%)] md:h-[calc(100vh-7%)] md:w-[calc(100vw-7%)] lg:h-[calc(100vh-20%)] lg:w-[calc(100vw-30%)] overflow-hidden flex flex-col">
          {isLoading ? (
            ...
            <div className="flex flex-1 inset-0 h-full overflow-hidden">
              <div className="hidden lg:flex flex-col justify-between w-[240px] border-r border-white-200 dark:border-dark-200 h-full px-3 pt-3 overflow-y-auto">
                <div className="flex flex-col">
                  <button
                    ...
                    className="group flex flex-row items-center hover:bg-white/5 dark:hover:bg-white/5 p-2 rounded-lg"
                  >
                    <ChevronLeft
                      size={18}
                  ...
                  <div className="flex flex-col items-start space-y-1 mt-8">
                    {sections.map((section) => (
                      <button
                        key={section.dataAdd}
                        className={cn(
                          `flex flex-row items-center space-x-2 px-2 py-1.5 rounded-lg w-full text-sm hover:bg-white/5 dark:hover:bg-white/5 transition duration-200 active:scale-95`,
                          activeSection === section.key
                            ? 'glass-panel text-black/90 dark:text-white/90'
              ...
              <div className="w-full flex flex-col overflow-hidden">
                <div className="flex flex-row lg:hidden w-full justify-between px-[20px] my-4 flex-shrink-0">
                  <button
                    onClick={() => setIsOpen(false)}
                    className="group flex flex-row items-center hover:bg-white/5 dark:hover:bg-white/5 rounded-lg mr-[40%]"
                  >
                    <ArrowLeft
                      size={18}
                      className="text-black/50 dark:text-white/50 group-hover:text-black/70 group-hover:dark:text-white/70"
