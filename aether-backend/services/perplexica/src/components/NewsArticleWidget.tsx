...
const NewsArticleWidget = () => {
  ...
  return (
    <div className="glass-panel rounded-2xl border border-light-200 dark:border-dark-200 shadow-sm shadow-light-200/10 dark:shadow-black/25 flex flex-row items-stretch w-full h-24 min-h-[96px] max-h-[96px] p-0 overflow-hidden">
      {loading ? (
        <div className="animate-pulse flex flex-row items-stretch w-full h-full">
          <div className="w-24 min-w-24 max-w-24 h-full glass-panel" />
          <div className="flex flex-col justify-center flex-1 px-3 py-2 gap-2">
            <div className="h-4 w-3/4 rounded glass-panel" />
            <div className="h-3 w-1/2 rounded glass-panel" />
          </div>
        </div>
      ) : error ? (
        ...
        >
          <div className="relative w-24 min-w-24 max-w-24 h-full overflow-hidden">
            <img
              className="object-cover w-full h-full glass-panel group-hover:scale-110 transition-transform duration-300"
              src={
