...
const NewsArticleWidget = () => {
  ...
  useEffect(() => {
    fetch('/api/discover?mode=preview&topic=legal')
      .then((res) => res.json())
      .then((data) => {
        const articles = (data.blogs || [])
          .map((a: any) => {
            ...
            const thumbCandidate = a.thumbnail || a.thumbnail_src || a.img_src || '';
            let thumbnail = '';
            try {
              if (thumbCandidate) {
            ...
            return { title, content, url, thumbnail } as Article;
          })
          .filter((a: Article) => a.thumbnail);
        setArticle(articles[Math.floor(Math.random() * articles.length)] || null);
        setLoading(false);
      })
      .catch(() => {
        setError(true);
