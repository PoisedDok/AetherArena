...
class OllamaLLM extends BaseLLM<OllamaConfig> {
  ...
  async generateObject<T>(input: GenerateObjectInput): Promise<T> {
    const messages = this.convertToOllamaMessages(input.messages);
    if (input.mode === 'json') {
      const schemaString = JSON.stringify(z.toJSONSchema(input.schema), null, 2);
    ...
    const response = await this.ollamaClient.chat({
      model: this.config.model,
      messages: messages,
      format: input.mode === 'json' ? 'json' : z.toJSONSchema(input.schema),
      ...(reasoningModels.find((m) => this.config.model.includes(m))
        ? { think: false }
        : {}),
      options: {
