...
import { extractiveIfNeeded } from '@/lib/utils/extractive';
      ...
      params.urls.map(async (url) => {
        try {
          ...
          if (url.startsWith('aether://')) {
            const apiBase = process.env.AETHER_BACKEND_URL || 'http://localhost:8765';
            const res = await fetch(`${apiBase}/v1/sources/document?url=${encodeURIComponent(url)}`);
            if (!res.ok) {
              throw new Error(`Aether backend returned ${res.status}: ${await res.text()}`);
            }
            const data = await res.json();
            text = data.content || '';
            title = data.title || `Aether Document: ${url}`;
          } else {
            const res = await fetch(url);
            text = await res.text();
            title = text.match(/<title>(.*?)<\/title>/i)?.[1] || `Content from ${url}`;
          }
          ...
          // Smart Pruning: remove obvious junk, then use DocumentUtility
          // extractive processing for large pages instead of hard truncation.
          // Hard truncation drops entire bottom of page (lossy).
          // Extractive processing ranks sections by importance and selects
          // the best ones within budget (preserves signal across full page).
          let prunedMarkdown = markdown
            .replace(/\[.*?\]\(javascript:.*?\)/g, '') // Remove JS links
            .replace(/!\[.*?\]\(.*?\)/g, ''); // Remove images to save tokens

          // Use DocumentUtility API for pages > 15k chars (instead of hard .substring)
          // Budget: 20k chars per URL (~5k tokens) — same as before, but intelligent
          prunedMarkdown = await extractiveIfNeeded(prunedMarkdown, {
            thresholdChars: 15000,
            budgetChars: 20000,
          });

          results.push({
            content: prunedMarkdown,
            metadata: {
              url,
              title: title,
            },
