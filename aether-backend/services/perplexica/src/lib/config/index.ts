...
class ConfigManager {
  ...
  /**
   * Deterministic IDs for providers we ship and control.
   * The Aether backend contract depends on these being stable across installs,
   * rebuilds, and volume resets.  User-added providers keep random UUIDs.
   */
  private static readonly BUILTIN_PROVIDER_IDS: Record<
    string,
  ...
  > = {
    ...
    transformers: {
      ...
      displayName: 'Transformers (ONNX)',
    },
  };

  private migrateConfig(config: Config): Config {
    // Migrate random UUIDs → deterministic IDs for built-in providers.
    // Existing installs may have been created before deterministic IDs were
    // enforced.  This ensures the backend's provider-ID contract is met even
    // after an in-place upgrade.
    for (const provider of config.modelProviders) {
      const builtin =
        ConfigManager.BUILTIN_PROVIDER_IDS[provider.type];
      if (builtin && provider.id !== builtin.id) {
        console.log(
          `[config migration] Upgrading provider ID: ${provider.id} → ${builtin.id} (type=${provider.type})`,
        );
        provider.id = builtin.id;
        provider.name = builtin.displayName;
      }
    }

    // Dedup: remove duplicate providers with the same ID.
    // Prior installs could accumulate duplicates (e.g. two aether-inference-default
    // entries with different baseURLs).  Keep the first per ID — that's the one
    // .find() resolves to, so removing later dupes is safe.
    const seenIds = new Set<string>();
    const deduped: typeof config.modelProviders = [];
    for (const provider of config.modelProviders) {
      if (seenIds.has(provider.id)) {
        console.log(
          `[config migration] Removing duplicate provider: id=${provider.id}, ` +
            `name=${provider.name}, baseURL=${provider.config?.baseURL ?? '(none)'}`,
        );
        continue;
      }
  ...
  private initializeFromEnv() {
    ...
    providerConfigSections.forEach((provider) => {
      const builtin =
        ConfigManager.BUILTIN_PROVIDER_IDS[provider.key];

      const newProvider: ConfigModelProvider & { required?: string[] } = {
        id: builtin?.id ?? crypto.randomUUID(),
        name: builtin?.displayName ?? `${provider.name}`,
        type: provider.key,
        chatModels: [],
        embeddingModels: [],
        config: {},
      ...
      if (configured) {
        ...
        // Dedup: for built-in providers check by deterministic ID first,
        // then fall back to hash match for user-added providers.
        const exists = this.currentConfig.modelProviders.find(
          (p) => (builtin && p.id === builtin.id) || p.hash === hash,
        );

        if (exists && builtin) {
          // Built-in provider already present — sync config from env
          // (URL may have changed between deployments).
          provider.fields.forEach((field) => {
            const envVal = process.env[field.env!];
            if (envVal) exists.config[field.key] = envVal;
          });
          exists.hash = hashObj(exists.config);
          exists.name = builtin.displayName;
        } else if (!exists) {
