...
const Page = () => {
  ...
  const fetchArticles = async (topic: string) => {
    ...
    try {
      ...
      const normalized = (data.blogs || [])
        .map((blog: any) => {
          // Normalize fields from searxng result shape
          const title = blog.title || blog?.name || '';
          const content = blog.content || blog?.snippet || '';
          const url = blog.url || blog?.link || '';
          const thumbCandidate =
            blog.thumbnail || blog.thumbnail_src || blog.img_src || '';
          let thumbnail = '';
          try {
            if (thumbCandidate) {
              const u = new URL(thumbCandidate);
              const id = u.searchParams.get('id');
              thumbnail = `${u.origin}${u.pathname}${id ? `?id=${id}` : ''}`;
            }
          } catch (_) {
            thumbnail = '';
          }
          return { title, content, url, thumbnail } as Discover;
        })
        .filter((blog: Discover) => !!blog.thumbnail);

      setDiscover(normalized);
    } catch (err: any) {
  ...
  // Auto-refresh every 30s for Legal topic only
  useEffect(() => {
    if (activeTopic !== 'legal') return;
