...
@.architecture
Incoming: security/auth.py::AuthenticationManager, api/dependencies.py --- {User, str role_name}
Processing: manage role registries, compute permission sets, authorize requests, log enforcement decisions --- {JOB_AUTHORIZE, JOB_LOG, JOB_MANAGE_ROLES, JOB_VALIDATE_CREDENTIALS}
Outgoing: AuthorizationContext, api/v1/endpoints/*.py --- {AuthorizationContext, bool permission_granted}
"""
...
class PermissionManager:
    ...
    def register_role(self, role_def: RoleDefinition) -> None:
        ...
        logger.info("Registered role '%s' with %d permissions", role_def.name, len(role_def.permissions))
...
class User:
    ...
    def __post_init__(self):
        ...
        if self.role not in [r.value for r in Role]:
            logger.warning("Unknown role '%s' for user %s", self.role, self.user_id)
