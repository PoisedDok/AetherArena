...
@.architecture
Incoming: security/crypto.py::SecretManager, security/permissions.py, api/dependencies.py --- {AuthConfig, http_headers}
Processing: manage API keys, govern tokens, authenticate requests, maintain sessions --- {JOB_AUTHORIZE, JOB_LOG, JOB_MANAGE_API_KEYS, JOB_MANAGE_SESSIONS, JOB_MANAGE_TOKENS, JOB_VALIDATE_CREDENTIALS}
Outgoing: security/permissions.py::AuthorizationContext, api/dependencies.py, in-memory registries --- {AuthorizationContext, Dict[str, Any]}
"""
...
class AuthenticationManager:
    ...
    def __init__(self, config: Optional[AuthConfig] = None):
        ...
        # API key registry: in-memory dict keyed by hashed API key.
        # Design choice: Aether runs as a single-instance desktop application
        # where the backend process owns all state. In-memory storage avoids
        # an external dependency for auth on every request. For multi-instance
        # or cloud deployment, swap to Redis/database-backed storage via the
        # same AuthenticationManager interface.
        self._api_keys: Dict[str, Dict[str, Any]] = {}
        
        # Active token registry: same single-instance rationale as API keys.
        # Tokens are short-lived (default 24h) and cleaned by cleanup_expired_tokens().
    ...
    def register_api_key(
        ...
        Args:
            ...
            api_key: Optional existing API key to import (plaintext)
            
        Returns:
            Generated API key (show only once to user)
        """
        # Generate API key if not provided
        api_key = api_key or self._hasher.generate_api_key(prefix="aether")
        api_key_hash = self._hasher.hash_token(api_key)
        
        # Avoid overwriting existing key registrations
        existing = self._api_keys.get(api_key_hash)
        if existing:
            logger.debug(
                "API key for user '%s' already registered; skipping duplicate import",
                existing["user_id"],
            )
            return api_key
        ...
        logger.info("Registered API key for user '%s' with role '%s'", user_id, role)
        return api_key
    ...
    def revoke_api_key(self, api_key: str) -> None:
        ...
        if api_key_hash in self._api_keys:
            ...
            logger.info("Revoked API key for user '%s'", self._api_keys[api_key_hash]["user_id"])
        else:
            logger.warning("Attempted to revoke unknown API key")
    ...
    def validate_api_key(self, api_key: str) -> User:
        ...
        logger.debug("Validated API key for user '%s'", user.user_id)
        return user
    ...
    def generate_token(
        ...
        expiry = datetime.now(timezone.utc) + timedelta(hours=self.config.token_expiry_hours)
        self._tokens[token] = {
            'user_id': user_id,
            'role': role,
            'metadata': metadata or {},
        ...
        logger.info("Generated token for user '%s' (expires: %s)", user_id, expiry)
        return token
    ...
    def validate_token(self, token: str) -> User:
        ...
        logger.debug("Validated token for user '%s'", user.user_id)
        return user
    ...
    def authenticate_request(
        ...
        if api_key:
            try:
                ...
                logger.warning("API key authentication failed: %s", e)
    ...
    def create_user_session(
        ...
        Returns:
            Session token
        """
        # Sessions are implemented as short-lived tokens (same in-memory store).
        # Single-instance desktop deployment: no external session store needed.
        # For distributed deployment, replace with Redis-backed session store.
