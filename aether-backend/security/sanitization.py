...
@.architecture
Incoming: api/v1/endpoints/*.py, user input streams --- {str text, bytes payload}
Processing: enforce size limits, sanitize text and paths, detect injection attempts --- {JOB_LOG, JOB_SANITIZE}
Outgoing: api/v1/endpoints/*.py --- {str sanitized_text, Path validated_path}
"""
...
class SizeLimits:
    ...
    # JSON payload limits
    MAX_JSON_SIZE_BYTES: int = 50 * 1024 * 1024  # 50MB
...
class InputSanitizer:
    ...
    def __init__(self, limits: Optional[SizeLimits] = None):
        ...
        self._sql_suspicious_tokens: List[str] = [
            "' or '",
            '" or "',
            "' or 1=1",
            '" or 1=1',
        ...
        self._sql_patterns: List[re.Pattern[str]] = [
            re.compile(r"('|\")\s*;\s*(select|insert|update|delete|drop|alter|create)\b", re.IGNORECASE),
    ...
    def sanitize_filename(self, filename: str) -> str:
        ...
        # Check for path traversal before stripping components
    ...
    def validate_url(
        ...
        if not allow_private_ips and parsed.hostname:
            if parsed.hostname.lower() == 'localhost':
                raise ValidationError("Private IP addresses not allowed: localhost")
            try:
    ...
    def check_sql_injection(self, text: str) -> bool:
        ...
        """
        lowered = text.lower()
        if any(token in lowered for token in self._sql_suspicious_tokens):
            return True
        return any(pattern.search(text) for pattern in self._sql_patterns)
