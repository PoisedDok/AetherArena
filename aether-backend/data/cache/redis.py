...
@.architecture
Incoming: app.py, api/dependencies.py, caching callers --- {str redis_url, Dict[str, Any] payload}
Processing: manage Redis connections, serialize cache entries, enforce TTL operations, run health probes --- {JOB_CLEANUP_RESOURCE, JOB_HEALTH_CHECK, JOB_MANAGE_CONNECTION, JOB_MANAGE_STORAGE}
Outgoing: Redis server, calling services --- {Redis command responses, Dict[str, Any] cache_state}
"""
...
class RedisCache:
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            # Reset state even on error to prevent stale connection flags
            self._client = None
            self._connected = False
    ...
    def raw_client(self) -> Optional[Any]:
        """Expose underlying Redis client (read-only)."""
        return self._client
    ...
    async def set(
        ...
        try:
            ...
            # Set with TTL if provided (use `is not None` so ttl=0 is honoured)
            if ttl is not None:
    ...
    async def increment(
        ...
        *,
        ...
        Returns:
            New counter value or None if operation failed
        """
        if not self.is_connected():
            return None
        ...
        try:
            ...
            if ttl is not None:
                await self._client.expire(namespaced_key, ttl)
            return int(result)
        except Exception as e:
    ...
    async def hset(
        ...
        try:
            namespaced_key = self._make_key(key)
            await self._client.hset(namespaced_key, mapping=mapping)
            if ttl is not None:
                await self._client.expire(namespaced_key, ttl)
            return True
        except Exception as e:
            logger.error(f"Failed to hset cache key '{key}': {e}")
            return False
    
    async def hgetall(self, key: str) -> Dict[str, Any]:
        """
        Retrieve all hash fields for a key.
        """
        if not self.is_connected():
            return {}
        ...
        try:
            namespaced_key = self._make_key(key)
            data = await self._client.hgetall(namespaced_key)
            return data or {}
        except Exception as e:
            logger.error(f"Failed to hgetall cache key '{key}': {e}")
            return {}
